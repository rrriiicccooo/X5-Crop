...
from ...policies.runtime_policy import OuterBoxDetectionPolicy
...
def base_outer_candidates(
    ...
    outer_policy = policy or OuterBoxDetectionPolicy()
    h, w = gray.shape
    bw = detect_outer(gray, outer_policy)
    white_x = detect_outer_white_x(gray, outer_policy)
    candidates = [OuterCandidate("bw", bw, "base_outer")]
    if white_x.valid():
        max_reasonable = max(
            float(bw.width) * outer_policy.white_x_width_multiplier,
            float(bw.width) + w * outer_policy.white_x_extra_ratio,
        )
        if white_x.width >= bw.width and white_x.width <= max_reasonable:
            candidates.append(OuterCandidate("white_x", white_x, "base_outer"))
    for profile in outer_policy.mask_profiles:
        box = detect_mask_profile_outer(gray, profile, outer_policy)
        if box is not None:
    ...
    non_full = [
        ...
        if (candidate.box.width * candidate.box.height) / max(1.0, canvas_area) <= outer_policy.candidate_max_area
    ]
    if non_full:
        return non_full
