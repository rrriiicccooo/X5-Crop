...
def choose_dual_lane_detection(
    ...
    if config.strip_mode != "full":
        return unsupported_dual_lane_partial_detection(gray, config)

    lanes = split_dual_lanes(cache.gray_work, policy.detector.dual_lane.lane_count)
    lane_detections = [
        detect_dual_lane(gray, config, lane, index, cache, policy)
        for index, lane in enumerate(lanes, start=1)
    ]
    return merge_dual_lane_detections(gray, config, lanes, lane_detections, policy)
