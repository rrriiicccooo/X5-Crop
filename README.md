...
- 120-66 / 120-67 已开始做更具体的参数校准：120-66 full 模式现在把三张 6x6 视为固定 count=3 的完整片条，并使用分隔优先 outer proposal：先从清晰的 66 黑色分隔带反推有效 outer，再交给原有 separator / edge-pair / scoring / review gate 判断；120-67 的横向画幅比例按 5:4 处理，并允许保守的 120-67 宽分隔 retry 和短轴 outer 收紧。
- V4.2.3 把“可信分隔 + format 几何反推 outer”的框架推广成 format-aware fallback：120-66 继续主动使用；135、half、xpan、120-645 和 120-67 只在常规 separator / wide retry 路径未能满足自动通过条件时才尝试，不覆盖已经可靠的正常检测结果。135-dual 因为是双条片夹的特殊 lane 逻辑，暂不启用这个通用 fallback。
- V4.2.4 清理了 separator-first fallback 的执行路径：fallback 现在只构建 separator-first outer 候选，不再重复把普通 outer 候选放进同一轮竞争；如果没有生成 separator-first outer，就安静回到原有 content / review 流程。同时 `separator_first_outer_mode` 会校验为 `off` / `fallback` / `always`，避免拼写错误静默改变检测行为。
- V4.2.5 为半格 full 增加保守的宽分隔 retry：只有在普通路径未通过、wide separator 数量足够、frame 宽度稳定、内容支撑正常且没有 equal fallback 时，才允许半格 full 通过自动裁切。wide 数量不足或仍主要依赖 grid / content 的图继续进入 REVIEW。
- V4.2.6 继续收紧半格 full 的候选选择：多数宽分隔、内容和几何都稳定的半格 full 可以通过；content run 明显 mismatch 时，不再让 content candidate 主导 REVIEW 展示，优先显示更可解释的 separator candidate。
- V4.2.7 增加半格 full 的稳定 grid 支持：当真实分隔只覆盖一部分，但 hard+grid 覆盖完整、frame 宽度极稳定、内容和 outer 对齐正常时，可以把稳定几何补位视为可靠证据。
- V4.2.8 改进启动器 partial mode 交互：只有开启 partial mode 后才询问 count；直接回车或输入 `auto` 表示自动判断，也可以输入当前 format 允许的固定张数。
...
不开启 partial 时，脚本会使用上表里的固定张数，速度更快，判断也更稳定。完整片条请优先保持 `partial mode = no`。开启 partial 时，启动器会继续询问 count：直接回车或输入 `auto` 代表自动判断；输入具体数字则固定局部张数。`135-dual` 目前只建议用于完整双条 135，partial 下会倾向复核。
...
Current active script version: V4.2.8
...
- The current detection logic, parameters, and regression tests are mainly
  optimized for normal 135 scans.
- 120-66 and 120-67 now have more specific tuning: 120-66 full mode treats the
  strip as a fixed count=3 set of three 6x6 frames and uses a separator-first
  outer proposal. It first uses clear 66 dark separator bands to infer the
  valid outer, then sends the candidate through the existing separator /
  edge-pair / scoring / review-gate pipeline. Horizontal 120-67 is handled as a
  5:4 frame, and 120-67 can use conservative wide-separator retry and
  short-axis outer tightening.
- V4.2.3 generalizes the "trusted separators + format geometry infer outer"
  framework into a format-aware fallback. 120-66 continues to use it actively;
  135, half-frame, xpan, 120-645, and 120-67 try it only when the normal
  separator / wide-retry path does not satisfy the auto-pass gate, so reliable
  existing detections are not overridden. 135-dual remains excluded because its
  two-lane strip logic is format-specific.
- V4.2.4 cleans up the separator-first fallback execution path: fallback now
  builds only separator-first outer candidates instead of rerunning ordinary
  outer candidates in the same retry. If no separator-first outer is generated,
  the detector quietly returns to the existing content / review flow. The
  `separator_first_outer_mode` policy is now validated as `off`, `fallback`, or
  `always` so typos cannot silently change detector behavior.
- V4.2.5 adds a conservative wide-separator retry for half-frame full strips.
  It can auto-approve only when the normal path fails, enough wide separators
  are found, frame widths are stable, content support is normal, and no equal
  fallback is used. Images with too few wide separators or mostly grid/content
  support still go to REVIEW.
- V4.2.6 further tightens half-frame full candidate selection: scans with
  majority wide separators plus stable content and geometry can pass, while
  REVIEW cases with clear content-run mismatch prefer the more explainable
  separator candidate instead of a misleading content candidate.
- V4.2.7 adds half-frame full stable-grid support: when true separators cover
  only part of the strip, but hard+grid covers all gaps, frame widths are very
  stable, and content/outer alignment is normal, stable geometry can count as
  reliable support.
- V4.2.8 improves launcher partial-mode interaction: the launcher asks for
  count only after partial mode is enabled; Return or `auto` keeps automatic
  count detection, and an allowed number fixes the partial frame count.
- Other formats already have format-aware policy entry points, but they have
  not been tuned image by image as carefully as 135.
- Higher-risk active features that are not enabled for other formats include
  nearby separator active correction, lucky-pass risk, and leading-grid
  failure. They are currently enabled only on the validated 135 path.
- For half / xpan / 120 formats, Debug Analysis diagnostics can help review the
  result, but auto-cropped results should still be checked carefully.
...
count. `135-dual` is currently recommended only for complete dual 135 strips;
partial dual-strip cases tend to be reviewed.
