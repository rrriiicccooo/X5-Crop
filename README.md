...
V3.1.1 检测方向仍然是候选竞争：脚本会在你指定的格式和片条模式内，为可能张数和不同证据来源生成多组候选裁切方案，再用几何、分隔、内容三类证据评分。最终输出的是得分最高且通过自动裁切闸门的候选；如果候选之间竞争接近，或者证据互相冲突，脚本会倾向于 `REVIEW`。

V3.1.1 从 V3 稳定基线继续优化，不继承 V3.1 的危险外框外扩逻辑。新增逻辑很窄：
...
如果扫描图缺少明显白色外框，或普通外框候选之间分歧明显，V3.1.1 会额外尝试用内部分隔条的等距关系反推外框边界。这个候选只用于竞争，不会直接覆盖普通外框，也不会扩到整张 TIFF 画布；普通外框稳定时会跳过这一步，避免给好图增加不必要的候选。
