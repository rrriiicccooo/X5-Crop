...
当前 active 脚本版本：V3.6.3

当前稳定发布版本：v3.3.1（GitHub Releases）

Current active script version: V3.6.3
...
| 版本 | 状态 | 检测逻辑 / 工作流变化 |
|---|---|---|
| V3.6.3 | 当前 active 开发版 | 将叠片 / 近似叠片视为困难图：135 full strip 中模型 gap 出现 strong overlap risk 时强制进入 REVIEW，不自动裁切。 |
| V3.6.2 | 开发版 | 诊断清理后的第一步逻辑瘦身：`equal-broad-region` 合并为普通 `equal`，`hard_fallback_detection` 缩小为 review-only equal split fallback；目标是减少低收益 method 和报告噪音。 |
...
V3.6.3 keeps bleed outside detection and treats overlap / near-overlap scans conservatively:
...
|---|---|---|
| V3.6.3 | Current active development | Treats overlap / near-overlap as difficult: in 135 full-strip mode, strong overlap risk on a model gap forces REVIEW instead of automatic export. |
| V3.6.2 | Development | First cleanup step after diagnostic stabilization: folds `equal-broad-region` into ordinary `equal`, and shrinks `hard_fallback_detection` to a review-only equal split fallback to reduce low-value methods and report noise. |
