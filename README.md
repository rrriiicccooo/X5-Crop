...
当前 active 脚本版本：V3.6.9

当前稳定发布版本：v3.6.2（GitHub Releases）

Current active script version: V3.6.9
...
| 版本 | 状态 | 检测逻辑 / 工作流变化 |
|---|---|---|
| V3.6.9 | 当前 active 开发版 | 统一 active grid protection 和 diagnostics 的轻量 hard-gap trust：grid 保护红框前也会识别 nearby conflict、geometry conflict、suspect internal edge / frame border，减少“双轨可信度”冲突。 |
...
|---|---|---|
| V3.6.9 | Current active development | Unifies active grid protection with lightweight diagnostic hard-gap trust: before grid can protect a red gap, it can now detect nearby conflicts, geometry conflicts, and suspected internal edge / frame border cases. |
