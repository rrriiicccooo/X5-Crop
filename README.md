...
当前检测方向是内容主导：脚本先用内容证据寻找真实照片矩形和画幅序列，再用分隔证据辅助确认。分隔线、黑条和 grid 不再优先决定裁切框；如果内容 run 数量和目标张数不一致，或者内容矩形证据不够稳定，脚本会倾向于 `REVIEW`。partial strip 不会因为“不完整”本身直接进入复核；只要内容、比例和辅助证据足够稳定，也可以自动通过。

Test 样本复盘后，当前算法不把“内容 run 数量”当作单一真相。真实照片内部的树枝、人物、墙面、窗框和高反差纹理会把一张照片切成多个内容峰；低纹理、欠曝或大面积平滑画面也会把多张照片合并成一个内容峰。

因此脚本会把内容矩形、目标画幅比例、完整/局部片条模型和分隔证据放在一起做联合判断。局部片条可以通过，但如果它会从一个仍然可信的完整片条模型手里“抢走结果”，脚本会改选完整模型并进入 `REVIEW`，避免困难图片错误自动裁切。
