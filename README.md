...
每张 Debug Analysis JPG 现在包含五个面板：

- `Decision summary`：版本号、PASS/REVIEW、confidence、format、strip、count、outer strategy、analysis source、auto gate、gap 证据和 review reasons。
- `Original gray`：原始灰度图。
- `Debug boxes`：外框和最终输出裁切范围。
- `Separator evidence`：整张扫描的分隔证据，并叠加当前 outer 和分隔标记。
- `Content evidence`：整张扫描的内容证据，并叠加当前 outer 和 frame。
...
Starting with V4.5, report details label the outer-candidate strategy, such as
...
maintenance cleanup, not a deliberate loosening of PASS rules.
...
Each Debug Analysis JPG now has five panels:

- `Decision summary`: version, PASS/REVIEW, confidence, format, strip, count,
  outer strategy, analysis source, auto gate, gap evidence, and review reasons.
...
- `Content evidence`: full-scan content evidence with the current outer and
  frames overlaid.
