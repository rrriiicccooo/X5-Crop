...
full 使用固定张数，partial 的目标是同时支持显式与自动 count；系统优先不切掉真实照片
内容，允许适量向外多保留，不要求唯一还原真实边界。
...
is intended to support either an explicit or automatic count. It prioritizes
preserving real photo content, permits modest outward over-retention, and does
not require a uniquely reconstructed physical boundary. The current V4.9
development build still emits source-core review evidence only and has not yet
implemented the new safe Grid/output contract, so it does not export individual
frame TIFFs. The stable release remains **v4.2.8**.
