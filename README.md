...
- 当外框候选不可靠时，脚本会尝试用已检测到的内部分隔条反推一个 `separator_derived_outer` 候选，让它和普通外框候选一起竞争；这个候选不能扩成整张画布，也不能大幅改变原外框。
...
如果扫描图缺少明显白色外框，V3.1.1 会额外尝试用内部分隔条的等距关系反推外框边界。这个候选只用于竞争，不会直接覆盖普通外框，也不会扩到整张 TIFF 画布。
