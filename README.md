...
普通用户请下载 GitHub Releases 里的 `X5-Crop-vX.X.zip`。不要下载 GitHub 自动生成的 `Source code` / 源码压缩包；源码包是开发结构，不是整理好的用户发布包。
...
1. 解压 Release 压缩包。
2. 第一次使用先运行 `install/` 里的安装启动器。
3. 把 `X5_Crop.py`、对应系统主启动器和 TIFF 长图放在同一个文件夹。
...
如果只想看最短步骤，请打开随包的 `快速启动_Quick_Start.md`。如果要查看版本差异和开发历史，请看 `CHANGELOG.md`。
...
- 当前稳定版：V4.0。V4.0 是模块化重写版，但 Release 用户仍然只需要单文件 `X5_Crop.py` 和对应系统启动器。全量 135 default-deskew dry run 对比 V3.9，`status`、`confidence`、`review_reasons`、`outer_box`、`frame_boxes` 和 `gaps` 为 0 diff。
- 上一稳定版：V3.6.2。它是诊断清理后的稳定发布版，但叠片、近似叠片、局部片距不稳定等困难图仍可能误识别。
- V3.7 到 V3.9 是为 V4 做准备的结构清理与 policy 迁移；V3.5、V3.4.x 等实验方向因为会影响部分准确样片，没有作为当前稳定路径。

更详细的开发记录、回滚背景和每个版本的检测逻辑变化见 `CHANGELOG.md`。
...
2. On first use, run the installer launcher inside `install/`.
3. Put `X5_Crop.py`, the platform main launcher, and TIFF scans in one folder.
4. Double-click the main launcher, then choose format, partial mode, and Debug
   Analysis.
...
For the shortest step-by-step guide, open `快速启动_Quick_Start.md`. For version
history and development details, see `CHANGELOG.md`.
...
- Current stable release: V4.0. V4.0 is the modular rewrite, but Release users
  still only need the standalone `X5_Crop.py` and the platform launcher. A full
  135 default-deskew dry run compared with V3.9 had 0 diffs for `status`,
  `confidence`, `review_reasons`, `outer_box`, `frame_boxes`, and `gaps`.
- Previous stable release: V3.6.2. It is the stable release after diagnostic
  cleanup, but overlap, near-overlap, and locally irregular spacing can still be
  misdetected.
- V3.7 through V3.9 prepared the V4 structure through policy migration and
  cleanup. V3.5 and V3.4.x were experimental directions that were not promoted
  because they affected some already-accurate scans.

For detailed development records, rollback context, and per-version detection
logic changes, see `CHANGELOG.md`.
