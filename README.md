...
格数；`full/partial` 不表示胶片是否铺满片夹。程序只在能够完整保护照片内容、限制多余边缘并保真写回 TIFF 时输出单张照片，否则将原
...
matched holder's full count. Full versus partial does not describe whether the
film fills the holder. It writes individual photos only when content
