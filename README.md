...
partial 片条还需提供明确的曝光格数。`full/partial` 只表示曝光格数是否完整，不表示胶片是否铺满
片夹。程序只在能够完整保护照片内容、限制多余边缘并保真写回 TIFF 时输出单张照片，否则将原
扫描件放入 `needs_review/`。仓库当前源码是唯一的 V5 current-only 实现；V5 尚未发布，当前公开
稳定版本仍为 **v4.2.8**。
...
for a partial strip. Full versus partial describes slot-count completeness, not
whether the film fills the holder. It writes individual photos only when content
protection, excess margins, and TIFF fidelity are all safe; otherwise it places
the source scan in `needs_review/`. The repository now contains one current-only
V5 implementation. The latest public stable release remains **v4.2.8**; V5 has
not been released.
