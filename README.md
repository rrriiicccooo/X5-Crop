...
Current development version: V3.4.1
...
| V3.4.2 | 已暂停 / 已回滚的开发尝试 | 尝试新增保守的局部 grid。实测会影响原本准确的样片，因此当前 active 脚本已暂时回滚到 V3.4.1。 |
| V3.4.1 | 当前开发版 | 强 hard separator 与 robust grid 冲突时优先保留红色证据，并把冲突记录到报告里，避免 grid 把可靠红框改写掉。 |
...
|---|---|---|
| V3.5 | Paused / rolled-back development attempt | Tried lightweight semantic validation for red hard separators. Testing affected previously accurate scans, so the active script is temporarily rolled back to V3.4.1. |
| V3.4.2 | Paused / rolled-back development attempt | Tried conservative local grid segments. Testing affected previously accurate scans, so the active script is temporarily rolled back to V3.4.1. |
| V3.4.1 | Current development | Keeps strong hard separators authoritative when they conflict with robust grid, and records the conflict in the report instead of rewriting reliable red evidence as grid. |
