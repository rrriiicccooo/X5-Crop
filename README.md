...
V3.1.2 检测方向仍然是候选竞争：脚本会在你指定的格式和片条模式内，为可能张数和不同证据来源生成多组候选裁切方案，再用几何、分隔、内容三类证据评分。最终输出的是得分最高且通过自动裁切闸门的候选；如果候选之间竞争接近，或者证据互相冲突，脚本会倾向于 `REVIEW`。

V3.1.2 从 V3 稳定基线继续优化，不继承 V3.1 的危险外框外扩逻辑。新增逻辑很窄：

- 当普通外框候选不稳定、缺少可信白边外框，或分隔模型显示外框可能偏移时，脚本会尝试用已检测到的内部分隔条反推一个 `separator_derived_outer` 候选，让它和普通外框候选一起竞争；这个候选不能扩成整张画布，也不能大幅改变原外框。
...
- 默认 bleed 改成“长轴更保护、短轴仍然收紧”：横向长图左右各 20px、上下各 10px；纵向长图上下各 20px、左右各 10px。
- 当 `same_frame_size_fit` 为了统一张宽而把首尾照片往外框内部缩得太多时，V3.1.2 会把首尾裁切范围重新贴回外框边缘，避免外框本来正确但第一张或最后一张被少裁一点。
- 对 135 完整片条里的 grid / equal 推断切线，脚本会在预测位置附近做一次很窄的局部分隔搜索；只有找到足够强的真实暗带证据时才升级为 detected，否则保持原来的 grid / equal，不凭空提高困难图置信度。
...
如果扫描图缺少明显白色外框，或普通外框候选之间分歧明显，V3.1.2 会额外尝试用内部分隔条的等距关系反推外框边界。这个候选只用于竞争，不会直接覆盖普通外框，也不会扩到整张 TIFF 画布；普通外框稳定时会跳过这一步，避免给好图增加不必要的候选。
