...
- 120-66 / 120-67 已开始做更具体的参数校准：120-66 full 模式现在把三张 6x6 视为固定 count=3 的完整片条，但允许有效 outer 在整张扫描长图内浮动，不再假设照片一定铺满或居中；120-67 的横向画幅比例按 5:4 处理，并允许保守的 120-67 宽分隔 retry 和短轴 outer 收紧。
...
Current active script version: V4.2.1
...
- 120-66 and 120-67 now have more specific tuning: 120-66 full mode treats the
  strip as a fixed count=3 set of three 6x6 frames, but allows the valid outer
  to float inside the scan instead of assuming the photos fill or sit centered
  in the whole long image; horizontal 120-67 is handled as a 5:4 frame, and
  120-67 can use conservative wide-separator retry and short-axis outer
  tightening.
