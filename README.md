...
- `快速启动_Quick_Start.md`：最短上手步骤，适合第一次使用或给他人转交工具。
- `README.md`：完整用户手册，说明安装、文件摆放、启动器、Debug Analysis、输出目录和命令行参数。
- `CHANGELOG.md`：开发记录和版本差异，适合排查行为变化、回滚或继续调检测逻辑。
...
- 不为了让困难图片通过而放宽置信规则。
- 困难样片进入复核，避免靠猜测自动裁切。
- 最终裁切 TIFF 尽量保持原 TIFF 的画质、位深、通道、ICC、resolution 和 metadata 行为。
