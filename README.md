...
当前 active 脚本版本：V3.6.2

当前稳定发布版本：v3.3.1（GitHub Releases）

Current active script version: V3.6.2
...
| 版本 | 状态 | 检测逻辑 / 工作流变化 |
|---|---|---|
| V3.6.2 | 当前 active 开发版 | 诊断清理后的第一步逻辑瘦身：`equal-broad-region` 合并为普通 `equal`，`hard_fallback_detection` 缩小为 review-only equal split fallback；目标是减少低收益 method 和报告噪音。 |
...
V3.6.2 keeps the V3.3.1 output baseline and keeps bleed outside detection:
...
|---|---|---|
| V3.6.2 | Current active development | First cleanup step after diagnostic stabilization: folds `equal-broad-region` into ordinary `equal`, and shrinks `hard_fallback_detection` to a review-only equal split fallback to reduce low-value methods and report noise. |
