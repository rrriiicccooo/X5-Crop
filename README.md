...
当前 active 脚本版本：V3.6.8

当前稳定发布版本：v3.6.2（GitHub Releases）

Current active script version: V3.6.8
...
| 版本 | 状态 | 检测逻辑 / 工作流变化 |
|---|---|---|
| V3.6.8 | 当前 active 开发版 | 将 V3.6.7 的 single-anchor 精确组合闸门改成 `lucky_pass_risk_score`：多项弱全局证据、可疑 hard gap、overlap model gap 和几何稳定性共同评分；超过阈值才进入 REVIEW。 |
...
|---|---|---|
| V3.6.8 | Current active development | Replaces V3.6.7's exact single-anchor gate with `lucky_pass_risk_score`: weak global evidence, suspicious hard gaps, overlap model gaps, and geometry stability are scored together; only scores above the threshold go to REVIEW. |
