...
1. 解压 Release 压缩包。
2. 把下面这些文件和要裁切的 TIFF 长图放在同一个文件夹里：
   - `X5_Crop.py`
   - macOS 用 `X5_Crop_Mac.command`
   - Windows 用 `X5_Crop_win.bat`
3. 双击对应系统的启动器。
4. 先选择胶片格式，再选择是否开启 partial mode 和 Debug Analysis。
...
2. Put these files in the same folder as the TIFF long-strip scans:
   - `X5_Crop.py`
   - macOS: `X5_Crop_Mac.command`
   - Windows: `X5_Crop_win.bat`
3. Double-click the launcher for your system.
4. Choose the film format, then choose whether to enable partial mode and Debug
   Analysis.
...
The Release zip contains only the script and the two main launchers:
