...
当前 active 脚本版本：V3.6.11

当前稳定发布版本：v3.6.2（GitHub Releases）

Current active script version: V3.6.11
...
| 版本 | 状态 | 检测逻辑 / 工作流变化 |
|---|---|---|
| V3.6.11 | 当前 active 开发版 | 将 `edge-pair` 从 135 专用扩展为 format-aware full-strip 逻辑：135 保持原参数，其它格式使用各自更保守的搜索窗口、gutter 宽度和质量阈值。 |
...
|---|---|---|
| V3.6.11 | Current active development | Extends `edge-pair` from 135-only to format-aware full-strip logic: 135 keeps its original parameters, while other formats use more conservative search windows, gutter widths, and quality thresholds. |
