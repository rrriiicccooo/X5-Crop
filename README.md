...
| 版本 | 状态 | 检测逻辑 / 工作流变化 |
|---|---|---|
| V4.0 | 当前 active 开发版 | 架构迁移兼容版：根目录 `X5_Crop.py` 变成薄入口，实际实现移动到 `x5crop/core.py`，并新增 `x5crop/io.py`、`policy.py`、`reports.py`、`deskew.py`、`detection/`、`debug/` 和 `regression.py` 作为清晰模块边界。第一版 V4 仍委托同一检测核心，目标是“新架构 + 旧结果”。全量 135 default-deskew dry run 对比 V3.9，`status`、`confidence`、`review_reasons`、`outer_box`、`frame_boxes` 和 `gaps` 为 0 diff。OpenCV / scipy 等大依赖后端暂定为未来 V5 方向。 |
...
|---|---|---|
| V4.0 | Current active development | Compatibility architecture migration: root `X5_Crop.py` is now a thin entry point, the implementation lives in `x5crop/core.py`, and `x5crop/io.py`, `policy.py`, `reports.py`, `deskew.py`, `detection/`, `debug/`, and `regression.py` define clearer module boundaries. The first V4 build still delegates to the same detection core: new architecture, old result. A full 135 default-deskew dry run compared with V3.9 had 0 diffs for `status`, `confidence`, `review_reasons`, `outer_box`, `frame_boxes`, and `gaps`. OpenCV / scipy-style heavy backends are deferred to a future V5 direction. |
