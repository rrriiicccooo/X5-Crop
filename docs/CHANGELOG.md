...
- 恢复简洁的 Debug Analysis 工作流：`--debug-analysis` 完整检测后只在 `x5_crop_output/` 发布
  诊断 JPG 与报告类文件，不写正式 TIFF 或 review copy；启动器不增加第二个 preview 选项。
  之后的普通运行自动验证并复用 current report 中的 Finalization facts；schema、完整性、版本、
  source runtime identity、TIFF profile、完整配置或 resolved layout 任一不匹配就重新检测。
- 删除独立 `--preview`、`x5_crop_preview/`、gzip detection snapshot、source-content SHA 与实现指纹
  路径。报告复用不加载 Git、cohort、comparator、profiling、receipt 或 fault injection；Pillow
  只在显式 Debug Analysis 时延迟导入。
