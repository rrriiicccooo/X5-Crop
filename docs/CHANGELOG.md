...
当前依赖合同跟随本机新版，NumPy 为 2.5.3、SciPy 为 1.18.1、tifffile 为 2026.9.20；后续升级核对实际版本并重新验证。

### 产品行为

- 固定 W 下的多个完整 H 解释可共享一份最终裁切，逐成员核对权限和原外扩预算，保留解释差异。
  仅在归属搜索、物化、比例、局部覆盖及边界用途全部闭合后进入原批准流程；内容检查、最终采样与报告
  绑定共同输出。证据不足、外扩超限或未覆盖的 enclosing-support 解释继续 Review。
- 短轴输出保护保留测量位置与方向的联合物理范围，并与统计拟合及逐点保护分别核对；
  避免拼接不可能同时成立的位置和方向，也保留统计位置窄带之外的合法物理状态。原预算与批准条件不变。
  两轴保护闭合后，按每个状态实际可达长轴端点重新投影上下边，保留全部已准入测量，减少对称最大
  外扩引入的多余保护；原左右保护、产品留边和连续状态覆盖保持。
...
- Cross 的物理不相容 component 按独立 anchor 与唯一相容 fragment 分组；每组完整 refit，失败保留全员，
  不丢弃测量或按结果挑选子集。完整组作为不可拆分输入，可追加完整并集支持的有条件裁切方案；
  原组及其首选、备选和批准状态保留；有条件方案单独占用至多一个待检查位置，不校准高度、不参与缺边精修，
  也不以外侧反证间接选择其他解。物理配对只枚举一次，两种权限视图的实际求解合计受原总上限约束。
  两种视图在离散选择前形成的全部 binding 分组现由运行时和开发报告保留，各组独立支持与条件依赖
  分开记录；不再因展示位置有限而丢失其余组。超界不发布部分集合，保留分组不改变当前裁切或批准条件。
  完整分组的原拟合未保留全部点时，可由同一物理可行域内的有限 Huber 拟合补充待检查方案；
  原始点、损失尺度和物理限制不变，原拟合与约束拟合的验收及实际工作量分别保存。
  完整原子的多重归属可形成有条件解释；同 lane 统一限制搜索访问，全部解释先通过整批拟合预算预检，
  再按完整 raw union 拟合。耗尽或超界时不添加该批提案，原方案继续保留；成员来源和实际搜索工作可复核。
  已唯一获权的直接上下边保留实测 H、完整测量保护与正常 bleed；不再因同一对线也包住名义 H 而转成
  enclosing 用途。直接 H 与已支持比例的冲突继续阻断批准，尚未直接闭合的完整外框证明保持原有职责。
  外框投影保留完整的上下位置与共同斜率状态，风险以短轴坐标单位计算；不遗漏斜率解、不重复添加 cross bleed。
  正常外扩后新进入范围的已登记端点与倾斜角点继续参与保护，实际采样、风险和对齐预算共用同一份结果。
  外框方向余量按各长轴状态的实际端点计算，保留长轴保护与完整测量账本，减少对称上界的多余留边。
  粗略外框各侧保留自身测量方向余量与完整共同方向，避免另一侧的额外误差传播到本侧域外保护；
  整条片带方向汇总仍覆盖两侧，报告逐侧核验来源。
  外框配对保留两侧各自完整的已接受测量，单侧首尾约束继续限制方向和输出；共同区域仍独立负责
  配对权限，不因另一侧缺少同位置测量而删除本侧证据。
  Aperture-center 校准同步登记当前 18 个 enclosing source 的观测指纹，资格及 `[-0.008H,+0.010H]`
  区间不变。
...
- Sequence 边界的完整 reference 位置保留同一组 raw physical 测量允许的全部直线状态，
  修复统计定位峰较窄或不对称时遗漏可行位置的问题。联合位置/斜率保留在测量事实中；
  canonical、拟合代表、角色资格与原角度上限不变，残差恢复不能重获方向权限。
- Aperture 裁切保护继续传播同一物理直线的位置/斜率关系，并闭合长短轴外扩对角点的相互影响；
  新进入保护范围的原始端点保留完整区间。Mandatory 与 requested 分别计算，bleed 不重复，
  原 5% 预算、候选数与像素查询上限不变。
- 晚期弱线投影使用完整 registered 身份账本，修复低采样占比但已有坐标权限的局部边被误报为
  unknown edge 的错误；phase seed 门槛、原全局 anchor、独立 rank 与弱线反证保持不变。
...
- Normal report 与 Debug Analysis 只显示同次检测事实，分开 proposal、candidate、正式输出、runner、
  ...
  `x5crop_v5_template_report_91`，移除已替代的同 pair enclosing 用途状态；Cross 分组保留逐成员来源、
  显式条件、完整归属搜索与拟合预算预检账本，
  以及原/约束拟合的独立验收；
  Sequence 完整位置回链原始物理直线集合，推导边保留共同 W 与方向的联合角点保护；
...
- Development gold 分开比较 proposal、candidate 与 approved output。Runtime 的实际数值预算评估和
  ...
  数值预算。Gold record / summary 为 v22 / v25；逐帧向内失败保留具体输出边和越界顶点，便于区分
  最终裁切保护失败与真实接触定位问题。原判定、坐标和预算不变。
