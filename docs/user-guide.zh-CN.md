...
上下画面边界已由独立证据唯一确定时，裁切保留实测高度及正常保护留量。即使这对边也能包住名义高度，
也不会改回框内推定高度。实测高度与已支持的比例约束冲突时仍需人工检查；无法直接确定画面边界时，
完整的外侧支撑证明仍可按原预算形成裁切。
...
报告记录原因，原有裁切方案继续保留；单个有条件组合不因此获得自动批准。
照片归属已确定且上下边有多种完整解释时，报告可另外记录覆盖这些解释的共同裁切范围及逐解释留边预算。
如果同一份输出在每个解释下均保护画面、留边合格，并通过全部证据与整张安全检查，可以自动批准，
无需强行确定唯一的上下边。解释不完整或任一检查不合格时仍进入 Review。
...
多余留边。粗略外框各侧保留自身测量方向余量及完整共同方向，避免另一侧的额外误差增加本侧留边。
各侧已接受的首尾测量也完整参与几何约束，即使另一侧没有同位置测量；配对仍须满足原有共同区域条件。
Support pair 已唯一选定后，程序可用黄金集校准且保留不确定性的 aperture-center 偏移区间
