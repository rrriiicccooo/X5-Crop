...
如果照片 top/bottom 只是两条局部直接边、没有覆盖整条 source，也没有形成三区域或 enclosing
support authority，而至少两个独立 separator/outer 位置已经一致闭合整条片带方向，程序使用这些
sequence 位置负责全局 deskew。局部 top/bottom 仍负责固定 H 和短轴位置，其方向偏离继续进入输出
保护与 5% 预算；不会把局部斜率外推成整条片带的旋转范围。保护或采样越界时仍为
`needs_review`。
