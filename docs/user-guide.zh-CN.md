...
已有完整边缘组还可共同形成待检查的裁切方案。每组原始测量完整保留；存在多种片段归属解释时，
报告记录分组条件，新方案保留为待检查提案。已有首选与备选方案及其批准条件保持不变；
新方案不会仅凭更多片段取得自动输出权限。
短轴裁切保护会同时考虑测量位置与方向的关联、统计拟合范围和局部边缘变化，保留需要覆盖的完整范围；
上下边按实际可达的左右端点计算保护，减少多余外扩。已有的逐侧外扩预算与自动批准条件继续适用。
...
报告保留每条 lane 已有的首选与单个备选裁切范围；长轴备选无法形成不同方案时，仍检查已有短轴备选。
另可保留一份有条件的短轴提案，与首选和备选分开显示。完整边界测量存在满足物理限制的拟合时，
这份提案可补充原拟合未能保留的整组测量解释，供人工检查；自动批准条件不变。
片段存在多重归属时，系统保留原完整测量组，并检查可能的整组组合。搜索或整批拟合超出工作上限时，
报告记录原因，原有裁切方案继续保留；这些组合只供 Review 检查。
无法生成时记录具体原因。开发黄金分析独立评价
...
照片 aperture 的保护范围包含斜边在四角的位移，以及长短轴外扩之间的相互影响；
完整保护范围和 bleed 一同接受逐侧预算检查。
这是当前模型的风险预算，不是对真实边界误差的直接测量。开发黄金另以人工基线检查最终裁切的方向性
包含与实际 5% 外扩；机器不必逐条复原人工线。源截断样片比较实际确认的裁剪后多边形，
逐侧诊断与完整包含判定一致，不要求保留 TIFF 外不存在的内容。
已确认唯一的一对照片上下边若同时满足完整外框支撑条件，输出直接使用这对实测边界，并保留全部位置与
倾斜不确定性。若直接观察到一对连续 outer support 完整包住固定 H，且总高度不超过 `1.1H`，它也可以替代不可用的
aperture top/bottom；这对直接支撑不依赖 W/H 比例推导。每侧必须有整条连续支撑，或有包围完整输出范围的
实测覆盖；只观察到末张照片前段不足以外推整段外框。此时不再添加 0.25 mm cross bleed，但逐侧与
同一可行状态的联合对齐 padding 仍分别受 5% 预算保护；support 位置不确定性只计入逐侧预算，不会在
联合项中重复计算，也不会相加不同状态的两侧极值。上下边的所有合法共同斜率参与角点保护，风险距离与
照片高度使用同一短轴单位。方向余量按各可行状态实际到达的长轴范围计算，完整保留测量保护并减少
多余留边。Support pair 已唯一选定后，程序可用黄金集校准且保留不确定性的 aperture-center 偏移区间
收窄最坏风险；它不会把 support 冒充照片边界。Calibration
