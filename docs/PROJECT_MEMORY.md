...
- 本地 `main` 尚未推到 `origin/main`。Pre-push 只运行 documentation 或 full，不再要求 performance
  receipt；performance 只在准备发布时绑定最终 release commit。
...
3. 任何 tracked 修复都会使旧 full 结果和 release performance receipt 失效；日常推送不因此受阻，
   但发布前必须在最终 commit 上重跑。

## 下一步

1. Fresh 运行九项黄金；按 selected chain、三轴 authority、content veto 与 envelope 逐项修复。
2. 先通过正常 hook 推送当前工程提交；黄金闭合后再为发布运行 111-source diagnostic、24-source
   performance 与平台验证。
