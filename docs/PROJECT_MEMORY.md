...
- Enclosing support 只证明 aperture 位于两条 support 之间，不证明 fixed `H` 居中。
  `EnclosingSupportApertureRisk` 对每个联合可行状态分别计算最不利 aperture 中心与 top/bottom 逐侧
  expansion，并消费原有 5% 预算；它不修改 output polygon，也不混合互斥状态。
