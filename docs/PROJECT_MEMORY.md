...
- 当前完整工作树 fresh `tools/verify full` 通过：325 tests，skip 2；compile、配置、cohort、shell 与
...
- 当前完整工作树 fresh 111-source diagnostic 全部完成且工程合同通过：23 张自动批准、88 张
  review。主要 blocking gap 是 sequence authority 29、cross authority 17、local advance 16 和
  离散 placement 15；该队列的 recognition accuracy 仍为 `not_assessed`。
- 最终 clean commit 的 24-source 完整路径性能通过：mean 约 4.5 秒/张，p50 约 3.9 秒，p95 约
  7.3 秒，最慢 S024 约 11.1 秒。阶段均值以 template alignment/decision 约 1.85 秒、sampling 约
  1.43 秒和 startup/import 约 1.25 秒为主；TIFF I/O 合计约 0.28 秒，不是当前最大热点。精确值
  只属于绑定 commit 的 performance receipt。
- 同一最终 commit 的 Apple Silicon platform I/O、APFS 与 HFS+ 本机验证通过；exFAT 仍按合同
  标记 best-effort unverified。该 receipt 不替代 Windows x64 或 Intel macOS 实机验证。

## 开放风险

- 当前性能平均值只比 5 秒上限低约 0.5 秒，且多张自动输出超过 5 秒；下一轮性能工作应优先看
  template alignment、sampling 和 startup，而不是凭直觉先改 TIFF I/O。
