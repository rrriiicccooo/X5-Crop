...
def detect_candidate_for_count(
    ...
    outer_candidates = outer_candidates + floating_full_outer_candidates(gray_work, outer_candidates, fmt, count, strip_mode)
    outer_candidates = outer_candidates + separator_first_outer_candidates(gray_work, outer_candidates, fmt, count, strip_mode, cache)
...
def floating_full_outer_candidates(
    ...
    tuning = format_tuning(fmt.name)
    if not tuning.floating_full_outer_enabled:
        return []
    if strip_mode != "full" or count != fmt.default_count:
        return []
    aspect = CONTENT_ASPECTS_HORIZONTAL.get(fmt.name)
    if aspect is None or aspect <= 0.0:
        return []
    ...
    for source in source_candidates:
        ...
        for extra in tuning.floating_full_outer_ratio_extras:
            ...
            for start in starts:
                ...
                candidates.append(
                    OuterCandidate(
                        f"floating_full_{source.name}_r{target_ratio:.3f}",
                        box,
                    )
                )
...
def separator_first_outer_candidates(
    ...
    if not tuning.separator_first_outer_enabled:
        return []
    if strip_mode != "full" or count != fmt.default_count or count != 3:
        return []
    aspect = CONTENT_ASPECTS_HORIZONTAL.get(fmt.name)
    if aspect is None or abs(float(aspect) - 1.0) > 1e-6:
        return []
    if not base_candidates:
        return []
    ...
    for source in source_candidates:
        ...
        if len(bands) < 2:
            continue
        bands = sorted(bands, key=lambda band: (-band["score"], band["center"]))[:10]
        pairs: list[tuple[float, dict[str, float], dict[str, float], float]] = []
        for i, first in enumerate(bands):
            for second in bands[i + 1:]:
                left_band, right_band = (first, second) if first["center"] <= second["center"] else (second, first)
                middle_width = right_band["start"] - left_band["end"]
                if middle_width <= 0:
                    continue
                spacing = right_band["center"] - left_band["center"]
                spacing_ratio = spacing / max(1.0, short_axis)
                if (
                    ...
                    continue
                frame_error = abs(middle_width - short_axis) / max(1.0, short_axis)
                if frame_error > tuning.separator_first_outer_frame_error_max:
                    continue
                proposed_left = int(round(outer.left + left_band["start"] - short_axis))
                proposed_right = int(round(outer.left + right_band["end"] + short_axis))
                ...
                expected_ratio = 3.0 + (left_band["width"] + right_band["width"]) / max(1.0, short_axis)
                ratio_error = abs(actual_ratio - expected_ratio)
                pair_score = ratio_error + frame_error - 0.02 * (left_band["score"] + right_band["score"])
                pairs.append((pair_score, left_band, right_band, expected_ratio))

        for rank, (_pair_score, left_band, right_band, expected_ratio) in enumerate(sorted(pairs, key=lambda item: item[0])[:4], start=1):
            proposed = Box(
                int(round(outer.left + left_band["start"] - short_axis)),
                outer.top,
                int(round(outer.left + right_band["end"] + short_axis)),
                outer.bottom,
            ).clamp(w, h)
            if not proposed.valid():
                continue
    ...
    return unique_outer_candidates(candidates)[: int(tuning.separator_first_outer_max_candidates)]
    ...
    for fmt in format_candidates:
        ...
        for count, strip_mode, offsets in count_specs:
            ...
            for offset in offsets:
                ...
                if (
                    strip_mode == "full"
                    and not separator_auto_gate
                    and tuning.wide_gap_retry_enabled
