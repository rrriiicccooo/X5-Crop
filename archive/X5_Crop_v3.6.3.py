...
HARD_GAP_METHODS = {"detected", "edge-pair", "enhanced-detected"}
...
def gap_diagnostic_record(gray_work: np.ndarray, detection: Detection, gap: Gap) -> dict[str, Any]:
    ...
    core_content = float((core < 235).mean())
    core_dark = float((core < 55).mean())
    core_activity = float(core.std() / 255.0)
    left_content = float((left < 235).mean()) if left.size else 0.0
    right_content = float((right < 235).mean()) if right.size else 0.0
    ...
    if gap.method in HARD_GAP_METHODS:
        if narrow_hard and content_continuous and weak_dark_gap:
            record["hard_trust"] = "suspect_internal_edge"
        elif dark_separator_like or core_content <= 0.08 or gap.score >= 0.45:
            record["hard_trust"] = "strong_separator_evidence"
        else:
            record["hard_trust"] = "uncertain_separator_evidence"
    elif gap.method in MODEL_GAP_METHODS:
        if dark_separator_like:
            overlap_risk = "none"
        elif continuity >= 0.70 and core_activity >= 0.12 and core_mean >= 55.0:
            overlap_risk = "strong"
        elif continuity >= 0.35 and core_activity >= 0.08 and core_mean >= 55.0:
            overlap_risk = "medium"
        elif continuity >= 0.16 and core_activity >= 0.04 and core_mean >= 55.0:
            overlap_risk = "weak"
        else:
            overlap_risk = "none"
...
def attach_read_only_diagnostics(gray: np.ndarray, detection: Detection) -> None:
    ...
    detection.detail["diagnostics_v3_6"] = {
        "version": "3.6.3",
        "diagnostic_only": True,
        "changes_output": False,
        "changes_confidence": False,
        "changes_pass_review": False,
        "purpose": "observe hard-gap trust, model-gap overlap risk, and evidence/model roles without changing V3.3.1 output",
...
def apply_overlap_review_gate(gray: np.ndarray, detection: Detection, threshold: float) -> None:
    if detection.film_format != "135" or detection.strip_mode != "full":
        detection.detail["overlap_review_gate"] = {"used": False, "reason": "not_135_full_strip"}
        return
    if detection.detail.get("analysis_source") == "hard_fallback":
        detection.detail["overlap_review_gate"] = {"used": False, "reason": "hard_fallback"}
        return

    gray_work = work_gray(gray, detection.layout)
    records = [gap_diagnostic_record(gray_work, detection, gap) for gap in detection.gaps]
    strong_records = [
        record for record in records
        if str(record.get("method")) in MODEL_GAP_METHODS and str(record.get("overlap_risk", "none")) == "strong"
    ]
    if not strong_records:
        detection.detail["overlap_review_gate"] = {
            "used": True,
            "triggered": False,
            "strong_overlap_model_gap_count": 0,
        }
        return

    detection.confidence = min(float(detection.confidence), min(0.84, threshold - 0.01))
    detection.review_reasons.append("overlap_or_near_overlap_review")
    detection.review_reasons = sorted(set(detection.review_reasons))
    detection.detail["overlap_review_gate"] = {
        "used": True,
        "triggered": True,
        "review_only": True,
        "reason": "strong_overlap_risk_on_model_gap",
        "strong_overlap_model_gap_count": len(strong_records),
        "gap_indexes": [int(record.get("index", -1)) for record in strong_records],
        "gap_methods": [str(record.get("method")) for record in strong_records],
        "confidence_cap": float(detection.confidence),
    }
...
def draw_gap_diagnostic_overlay(rgb: np.ndarray, detection: Detection, scale: float) -> None:
    ...
    for record in records:
        ...
        if record.get("hard_trust") == "suspect_internal_edge":
            color = (255, 0, 220)
        elif str(record.get("overlap_risk", "none")) == "strong":
            color = (0, 220, 255)
        if color is None:
            continue
        for tick in gap_tick_boxes(detection, gap):
            if detection.layout == "horizontal":
                draw_preview_line(rgb, tick, scale, color, 3)
            else:
                draw_preview_hline(rgb, tick, scale, color, 3)
...
def process_one(input_file: Path, config: Config) -> ProcessResult:
    ...
    apply_overlap_review_gate(gray, detection, config.confidence_threshold)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="X5 Crop V3.6.3 overlap-review single-strip TIFF film cropper.")
