...
def gap_diagnostic_record(gray_work: np.ndarray, detection: Detection, gap: Gap) -> dict[str, Any]:
    ...
    if gap.method in HARD_GAP_METHODS:
        ...
        elif core_mean <= 55.0 or core_content <= 0.08 or gap.score >= 0.45:
        ...
        record["overlap_like"] = bool(continuity >= 0.14 and core_activity >= 0.028 and core_mean >= 60.0)
    return record
...
def draw_gap_diagnostic_overlay(rgb: np.ndarray, detection: Detection, scale: float) -> None:
    ...
    for record in records:
        ...
        if record.get("hard_trust") == "suspect_internal_edge":
            color = (255, 0, 220)
        elif bool(record.get("overlap_like", False)):
            color = (0, 220, 255)
        if color is None:
            continue
        for tick in gap_tick_boxes(detection, gap):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="X5 Crop V3.6 diagnostic-cleanup single-strip TIFF film cropper.")
