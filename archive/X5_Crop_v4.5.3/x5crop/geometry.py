...
def cached_separator_profile(cache: Optional[AnalysisCache], gray_work: np.ndarray, outer: Box, format_name: str = "135") -> np.ndarray:
    ...
    if is_full_work_box(cache.gray_work, outer):
        profile = cache.separator_profiles_full.get(format_name)
        if profile is None:
            profile = separator_profile(cache.gray_work, format_name)
            cache.separator_profiles_full[format_name] = profile
            cache.separator_profiles[format_box_cache_key(format_name, full_work_box(cache.gray_work))] = profile
        return profile
...
def cached_enhanced_separator_profile(cache: Optional[AnalysisCache], gray_work: np.ndarray, outer: Box, format_name: str = "135") -> np.ndarray:
    ...
    if is_full_work_box(cache.gray_work, outer):
        profile = cache.enhanced_separator_profiles_full.get(format_name)
        if profile is None:
            profile = separator_profile(cached_full_separator_evidence(cache, cache.gray_work), format_name)
            cache.enhanced_separator_profiles_full[format_name] = profile
            cache.enhanced_separator_profiles[format_box_cache_key(format_name, full_work_box(cache.gray_work))] = profile
        return profile
    key = format_box_cache_key(format_name, outer)
...
def merge_enhanced_separator_gaps(
    ...
    if cache is not None:
        cache_key = enhanced_separator_merge_cache_key(outer, gaps, origin, pitch, strip_mode, format_name)
