...
OUTER_PROPOSAL_MODES = {"off", "fallback", "conditional", "always"}
...
class FormatTuning:
    ...
    score_gate_135_min_hard_gaps: int = 2
    score_gate_135_max_equal_min: int = 2
    score_gate_135_low_hard_cap: float = 0.82
    score_gate_135_mostly_equal_cap: float = 0.84
    ...
    score_gate_135_allow_full_detected_geometry: bool = True
    score_gate_half_allow_geometry: bool = True
    ...
    separator_gate_120_edge_pair_min_score_without_wide: float = 0.0
    separator_gate_120_edge_pair_min_score_with_wide: float = 0.0
    separator_gate_120_min_wide_gaps_for_auto: int = 0
...
@dataclass(frozen=True)
class OuterPolicyView:
    """Read-only grouped view over outer extraction and alignment tuning."""
...
class ContentPolicyView:
    """Read-only grouped view over content evidence tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class SeparatorPolicyView:
    """Read-only grouped view over separator and gap tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class GridPolicyView:
    """Read-only grouped view over robust grid tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class ScoringPolicyView:
    """Read-only grouped view over raw score and support-score tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class CalibrationPolicyView:
    """Read-only grouped view over candidate calibration and auto-gate tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class PartialPolicyView:
    """Read-only grouped view over partial-strip tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class DiagnosticsPolicyView:
    """Read-only grouped view over diagnostics and overlap-risk tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class DebugPolicyView:
    """Read-only grouped view over Debug Analysis display tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class DeskewPolicyView:
    """Read-only grouped view over deskew tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class OuterProposalPolicyView:
    """Read-only grouped view over outer proposal strategy tuning."""

    tuning: FormatTuning


@dataclass(frozen=True)
class FormatPolicyViews:
    """Grouped policy views used to make FormatTuning easier to navigate.

    The underlying fields stay on FormatTuning for backward compatibility with
    archived scripts and the generated standalone release script. These views
    are intentionally thin and behavior-preserving.
    """

    outer: OuterPolicyView
    content: ContentPolicyView
    separator: SeparatorPolicyView
    grid: GridPolicyView
    scoring: ScoringPolicyView
    calibration: CalibrationPolicyView
    partial: PartialPolicyView
    diagnostics: DiagnosticsPolicyView
    debug: DebugPolicyView
    deskew: DeskewPolicyView
    outer_proposals: OuterProposalPolicyView


def policy_views(tuning: FormatTuning) -> FormatPolicyViews:
    return FormatPolicyViews(
        outer=OuterPolicyView(tuning),
        content=ContentPolicyView(tuning),
        separator=SeparatorPolicyView(tuning),
        grid=GridPolicyView(tuning),
        scoring=ScoringPolicyView(tuning),
        calibration=CalibrationPolicyView(tuning),
        partial=PartialPolicyView(tuning),
        diagnostics=DiagnosticsPolicyView(tuning),
        debug=DebugPolicyView(tuning),
        deskew=DeskewPolicyView(tuning),
        outer_proposals=OuterProposalPolicyView(tuning),
    )
...
def separator_first_outer_mode_for_strip(tuning: FormatTuning, strip_mode: str) -> str:
    if strip_mode == "partial":
        mode = str(tuning.separator_first_partial_mode).strip().lower()
        if mode not in SEPARATOR_FIRST_OUTER_MODES:
            raise ValueError(f"invalid separator_first_partial_mode: {tuning.separator_first_partial_mode!r}")
        if not tuning.separator_first_partial_enabled:
            return "off"
        return mode
    return separator_first_outer_mode(tuning)


def separator_geometry_outer_mode_for_strip(tuning: FormatTuning, strip_mode: str) -> str:
    mode_value = tuning.separator_geometry_outer_partial_mode if strip_mode == "partial" else tuning.separator_geometry_outer_full_mode
    mode = str(mode_value).strip().lower()
    if mode not in OUTER_PROPOSAL_MODES:
        raise ValueError(f"invalid separator_geometry_outer mode: {mode_value!r}")
    return mode
...
def long_axis_edge_anchor_outer_mode_for_strip(tuning: FormatTuning, strip_mode: str) -> str:
    if strip_mode == "partial":
        mode = str(tuning.long_axis_edge_anchor_partial_mode).strip().lower()
        if mode not in SEPARATOR_FIRST_OUTER_MODES:
            raise ValueError(f"invalid long_axis_edge_anchor_partial_mode: {tuning.long_axis_edge_anchor_partial_mode!r}")
        if not tuning.long_axis_edge_anchor_partial_enabled:
            return "off"
        return mode
    return long_axis_edge_anchor_outer_mode(tuning)
...
class AnalysisCache:
    ...
    content_mask_details: dict[str, dict[str, Any]] = field(default_factory=dict)
