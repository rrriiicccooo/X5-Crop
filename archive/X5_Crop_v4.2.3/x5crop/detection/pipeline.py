...
def detect_candidate_for_count(
    ...
    cache: Optional[AnalysisCache] = None,
    gap_max_width_ratio_override: Optional[float] = None,
    force_separator_first: bool = False,
) -> Detection:
    gray_work = cache.gray_work if cache is not None and cache.layout == config.layout else work_gray(gray, config.layout)
    ...
    tuning = format_tuning(fmt.name)
    if force_separator_first or tuning.separator_first_outer_mode == "always":
        ...
        for count, strip_mode, offsets in count_specs:
            ...
            for offset in offsets:
                ...
                if (
                    ...
                    and tuning.separator_first_outer_mode == "fallback"
                ):
                    separator_first = detect_candidate_for_count(
                        gray,
                        config,
                        fmt,
                        count,
                    ...
                    separator_first_candidate.detail["separator_first_outer_retry"] = {
                        "used": True,
                        "mode": str(tuning.separator_first_outer_mode),
                    }
