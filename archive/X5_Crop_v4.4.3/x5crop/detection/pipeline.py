...
def separator_first_band_sequences(
    ...
    if expected_gaps == 2:
        ...
        return sorted(
            pairs,
            key=lambda pair: -0.5 * (float(pair[0]["score"]) + float(pair[1]["score"])),
...
def partial_safe_auto_candidate(detection: Detection, threshold: float) -> bool:
    candidate = detection.detail.get("v2_candidate", {})
    return bool(
    ...
    for count, strip_mode, offsets in count_specs:
        ...
        for offset in offsets:
            ...
            if (
                ...
                if bool(wide_candidate.detail.get("v2_candidate", {}).get("auto_gate", False)):
            ...
            partial_safe_auto = partial_safe_auto_candidate(separator_gate_candidate, config.confidence_threshold)
