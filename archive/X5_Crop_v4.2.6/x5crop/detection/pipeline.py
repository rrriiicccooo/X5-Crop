...
def detect_candidate_for_count(
    ...
    tuning = format_tuning(fmt.name)
    if separator_first_outer_mode(tuning) == "always":
...
def detect_separator_first_candidate_for_count(
    gray: np.ndarray,
    config: Config,
    fmt: FilmFormat,
    count: int,
    ...
    gray_work = cache.gray_work if cache is not None and cache.layout == config.layout else work_gray(gray, config.layout)
    base_outer_candidates = detect_outer_candidates(gray_work, fmt.name)
    base_outer_candidates = base_outer_candidates + floating_full_outer_candidates(
        gray_work,
        base_outer_candidates,
        fmt,
        count,
        strip_mode,
    )
    tuning = format_tuning(fmt.name)
    outer_candidates = separator_first_outer_candidates(gray_work, base_outer_candidates, fmt, count, strip_mode, cache)
...
def separator_first_outer_candidates(
    ...
    if not tuning.separator_first_outer_enabled:
        return []
    if strip_mode != "full" or count != fmt.default_count or count <= 1:
        return []
    aspect = CONTENT_ASPECTS_HORIZONTAL.get(fmt.name)
    if aspect is None or aspect <= 0.0:
        return []
        ...
        for count, strip_mode, offsets in count_specs:
            ...
            for offset in offsets:
                ...
                if (
                    strip_mode == "full"
                    and not separator_auto_gate
                    and tuning.separator_first_outer_enabled
                    and separator_first_outer_mode(tuning) == "fallback"
                ):
                    separator_first = detect_separator_first_candidate_for_count(
                        gray,
                        config,
                        fmt,
                        count,
                    ...
                    if separator_first is not None:
                        separator_first_candidate = calibrate_v2_candidate(gray, separator_first, config, fmt, "separator", cache)
                        separator_first_candidate.detail["separator_first_outer_retry"] = {
                            "used": True,
                            "mode": separator_first_outer_mode(tuning),
                        }
                        candidates.append(separator_first_candidate)
                        separator_first_auto_gate = bool(
                            separator_first_candidate.detail.get("v2_candidate", {}).get("auto_gate", False)
                        )
                        if separator_first_auto_gate:
                            separator_auto_gate = True
                            separator_gate_candidate = separator_first_candidate
