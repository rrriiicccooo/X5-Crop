...
def edge_pair_params_for_format(format_name: str) -> EdgePairParams:
    if format_name in {"135", "135-dual"}:
        return EdgePairParams(0.080, 0.004, 0.050, 0.42, 0.62, 0.0)
    if format_name == "half":
        return EdgePairParams(0.090, 0.003, 0.060, 0.46, 0.66, 1.05)
    if format_name == "xpan":
        return EdgePairParams(0.060, 0.002, 0.035, 0.45, 0.64, 1.03)
    if format_name == "120-645":
        return EdgePairParams(0.070, 0.002, 0.035, 0.44, 0.63, 1.02)
    if format_name in {"120-66", "120-67"}:
        return EdgePairParams(0.070, 0.002, 0.032, 0.44, 0.63, 1.02)
    return EdgePairParams(0.070, 0.003, 0.040, 0.45, 0.64, 1.05)
    ...
    *,
    ...
    if gray_work is not None and outer is not None and gap.start is not None and gap.end is not None:
        ...
        if core.size:
            ...
            if width_ratio < 0.010 and dark_separator_like:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="X5 Crop V3.6.11 single-strip TIFF film cropper.")
