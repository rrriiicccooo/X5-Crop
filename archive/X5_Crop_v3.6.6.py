...
def grid_protection_trust(gap: Gap, pitch: float) -> str:
...
def apply_robust_grid(gaps: list[Gap], origin: float, pitch: float, strip_mode: str) -> tuple[list[Gap], dict[str, Any]]:
    ...
    for gap in constrained:
        ...
        if gap.method in {"detected", "edge-pair", "enhanced-detected"} and abs(gap.center - predicted) <= max(3.0, pitch * 0.025):
            adjusted.append(gap)
        elif allow_hard_protection and grid_protection_trust(gap, pitch) == "strong_separator":
            adjusted.append(gap)
                ...
                overridden_hard.append(
                    {
                        ...
                        "width_px": float(gap.width),
                        "score": float(gap.score),
                        "trust": grid_protection_trust(gap, pitch),
                    }
                )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="X5 Crop V3.6.6 hard-gap trust diagnostics single-strip TIFF film cropper.")
