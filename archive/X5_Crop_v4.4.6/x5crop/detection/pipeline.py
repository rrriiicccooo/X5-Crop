...
def separator_confirmed_partial_cache_key(
    base_candidates: list[OuterCandidate],
    fmt: FilmFormat,
    count: int,
    strip_mode: str,
...
def separator_confirmed_partial_can_compete(detection: Detection, gray: np.ndarray) -> bool:
...
def detect_candidate_for_count(
    ...
    regular_best = max(candidates, key=lambda d: detection_rank(d, config.confidence_threshold)) if candidates else None
    if regular_best is None or separator_confirmed_partial_can_compete(regular_best, gray):
        confirmed_outer_candidates = separator_confirmed_partial_outer_candidates(
            gray_work,
            outer_candidates,
            fmt,
            count,
        ...
        for candidate in confirmed_outer_candidates:
            candidate_gap_override = gap_max_width_ratio_override
            if (
        ...
        outer_candidates = unique_outer_candidates([*outer_candidates, *confirmed_outer_candidates])
...
def separator_first_band_sequences(
...
def separator_confirmed_partial_outer_candidates(
    gray_work: np.ndarray,
    base_candidates: list[OuterCandidate],
    fmt: FilmFormat,
    count: int,
    ...
    tuning = format_tuning(fmt.name)
    if not tuning.separator_confirmed_partial_outer_enabled:
        return []
    if strip_mode != "partial" or count <= 1:
        return []
    expected_count = int(tuning.separator_confirmed_partial_outer_count)
    ...
    if not base_candidates:
        return []
    if cache is not None:
        candidate_key = separator_confirmed_partial_cache_key(base_candidates, fmt, count, strip_mode)
        cached_candidates = cache.separator_confirmed_partial_outer_candidates.get(candidate_key)
        if cached_candidates is not None:
            return list(cached_candidates)
    ...
    source_candidates = sorted(
        ...
        key=lambda candidate: candidate.box.width * candidate.box.height,
        reverse=True,
    )[: max(1, int(tuning.separator_confirmed_partial_outer_source_candidates))]
    candidates: list[OuterCandidate] = []
    ...
    for source in source_candidates:
        ...
        for sequence in separator_first_band_sequences(bands, expected_gaps, frame_long, tuning):
            ...
            for margin_ratio in tuning.separator_confirmed_partial_outer_margin_ratios:
        ...
        for rank, (_sequence_rank, sequence, expected_ratio) in enumerate(
            sorted(sequences, key=lambda item: item[0])[: max(1, int(tuning.separator_confirmed_partial_outer_max_candidates))],
            start=1,
        ):
            first_band = sequence[0]
            last_band = sequence[-1]
            ...
            candidates.append(
                OuterCandidate(
                    f"separator_confirmed_partial_{source.name}_{rank}_r{expected_ratio:.3f}",
                    proposed,
                )
            )

    result = unique_outer_candidates(candidates)[: int(tuning.separator_confirmed_partial_outer_max_candidates)]
    if cache is not None:
        cache.separator_confirmed_partial_outer_candidates[candidate_key] = list(result)
    return result
...
def detect_dual_135_lane(
    ...
    if not candidates:
        return None
    best = max(candidates, key=lambda d: candidate_selection_rank(d, config.confidence_threshold))
...
def candidate_counts_for_format(config: Config, fmt: FilmFormat) -> list[tuple[int, str, tuple[float, ...]]]:
    def candidate_offsets(count: int) -> tuple[float, ...]:
    ...
    if config.strip_mode == "partial":
        if config.count_override is not None:
            return [(config.count, "partial", candidate_offsets(config.count))]
        tuning = format_tuning(fmt.name)
        return [
            (count, "partial", candidate_offsets(count))
            for count in partial_candidates(fmt)
...
def calibrate_candidate_decision(
    ...
    if not auto_gate:
        ...
        reasons.append("auto_gate_not_satisfied")
    else:
    ...
    candidate.detail["analysis_source"] = f"{source}_candidate"
...
def candidate_selection_rank(detection: Detection, threshold: float) -> tuple[int, float, int, float]:
    candidate = detection.detail.get("candidate_decision", {})
...
def choose_detection(gray: np.ndarray, config: Config, fmt: FilmFormat, cache: Optional[AnalysisCache] = None) -> Detection:
    ...
    for count, strip_mode, offsets in count_specs:
        ...
        for offset in offsets:
            ...
            separator_candidate = calibrate_candidate_decision(gray, separator, config, fmt, "separator", cache)
            ...
            if (
                ...
                wide_candidate = calibrate_candidate_decision(gray, wide_separator, config, fmt, "separator", cache)
            ...
            if (
                ...
                if fallback_proposal is not None:
                    fallback_candidate = calibrate_candidate_decision(gray, fallback_proposal, config, fmt, "separator", cache)
            ...
            if content is not None:
                candidates.append(calibrate_candidate_decision(gray, content, config, fmt, "content", cache))
    ...
    candidates = sorted(candidates, key=lambda d: candidate_selection_rank(d, config.confidence_threshold), reverse=True)
    best = candidates[0]
    selection_override: Optional[str] = None
    half_separator_review = select_half_separator_review_candidate(best, candidates)
