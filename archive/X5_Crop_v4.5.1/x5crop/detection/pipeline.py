...
def outer_proposal_strategy_plan(
    ...
    base = [
        ...
        OuterProposalStrategy("content_floating", "content_floating_outer", "always", False, "medium"),
    ]
    active = [
        OuterProposalStrategy(
            "long_axis_edge_anchor",
            "long_axis_edge_anchor_outer",
...
def calibrated_candidates_for_count(
    ...
    wide_retry_allowed = wide_separator_retry_allowed_for_strip(tuning, strip_mode)
...
def choose_detection(gray: np.ndarray, config: Config, fmt: FilmFormat, cache: Optional[AnalysisCache] = None) -> Detection:
    ...
    return select_detection_candidate(candidates, fmt, config.confidence_threshold)
