...
def outer_proposal_candidates(
    ...
    if long_axis_edge_anchor_outer_mode(tuning) == include_long_axis_edge_anchor_mode:
    ...
    if separator_first_outer_mode(tuning) == include_separator_first_mode:
    ...
    for source in source_candidates:
        ...
        for extra in tuning.long_axis_edge_anchor_ratio_extras:
            ...
            for anchor_name, left, right in anchors:
                ...
                candidates.append(
                    OuterCandidate(
                        f"long_axis_edge_anchor_{anchor_name}_{source.name}_r{target_ratio:.3f}",
                        box,
                    )
                )
        ...
        for count, strip_mode, offsets in count_specs:
            ...
            for offset in offsets:
                ...
                if (
                    ...
                    and not separator_auto_gate
                    and (
                        separator_first_outer_mode(tuning) == "fallback"
                        or long_axis_edge_anchor_outer_mode(tuning) == "fallback"
                    )
                ):
                    ...
                    if fallback_proposal is not None:
                        ...
                        fallback_candidate.detail["outer_proposal_fallback_retry"] = {
                            "used": True,
                            "separator_first_mode": separator_first_outer_mode(tuning),
                            "long_axis_edge_anchor_mode": long_axis_edge_anchor_outer_mode(tuning),
                        }
