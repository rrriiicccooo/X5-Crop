...
PARAMETER_GROUPS = (
    ...
    _group(FormatPhysicalSpec, ("default_count", "lane_count"), ParameterRole.PHYSICAL_FACT, "count", "format", "Physical strip and lane counts."),
    _group(FormatPhysicalSpec, ("allowed_counts",), ParameterRole.PHYSICAL_FACT, "count_set", "format", "Physically supported frame counts."),
    _group(FormatPhysicalSpec, ("complete_strip_can_be_underfilled",), ParameterRole.PHYSICAL_FACT, "boolean", "format", "Whether a complete strip may occupy less than the holder span."),
    ...
    _group(ContentEvidenceImageParameters, ("gradient_percentile", "texture_percentile", "local_contrast_percentile"), ParameterRole.ADAPTIVE_MEASUREMENT, "percentile", "preprocess", "Per-image content evidence normalization."),
    ...
    _group(BoundaryPathParameters, ("maximum_section_width_ratio_to_scan_extent", "minimum_path_support_ratio", "inner_sample_ratio"), ParameterRole.ADAPTIVE_MEASUREMENT, "ratio", "boundary_path", "Scale-independent local boundary-path sampling and aggregation."),
    ...
    _group(BoundaryPathParameters, ("strongest_change_points_per_section",), ParameterRole.ADAPTIVE_MEASUREMENT, "count", "boundary_path", "Keeps the strongest local changes before cross-section path fitting."),
    _group(BoundaryPathParameters, ("maximum_paths_per_axis",), ParameterRole.EXECUTION_BUDGET, "count", "boundary_path", "Bounds count-independent boundary-path expansion."),
