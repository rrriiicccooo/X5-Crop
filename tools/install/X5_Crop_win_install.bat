...
if not exist "install\dependencies.toml" (
    echo Missing setup file: install\dependencies.toml
    echo.
    pause
    exit /b 1
)
...
:use_fallback
if not defined PYTHON_BASE if defined PYTHON_FALLBACK set "PYTHON_BASE=%PYTHON_FALLBACK%"
exit /b 0
...
if not defined PYTHON_FALLBACK set "PYTHON_FALLBACK=%CANDIDATE%"
%CANDIDATE% "install\dependency_manager.py" check --contract "install\dependencies.toml" --quiet >nul 2>nul
if "%errorlevel%"=="0" set "PYTHON_BASE=%CANDIDATE%"
exit /b 0
