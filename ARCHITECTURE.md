...
| Gap / separator | robust grid model gaps | `detection/candidate/proposal/separator/refinement.py`, `geometry/gap_geometry.py`, `geometry/robust_grid.py` | grid 只能补模型证据；weak grid 不得单独获得 auto PASS；grid 已拆成 reliable anchor selection、grid fit、predicted center 和 hard-gap protection / override adjustment；gap geometry helper 只提供几何约束，不寻找 separator；robust grid 不接收 format identity，只消费 gaps、origin/pitch、strip mode 和参数对象，方便单独审核 model gap 何时能覆盖弱 hard gap。 |
