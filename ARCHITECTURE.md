...
| Gap / separator | separator cache | `cache/separator.py`, `detection/evidence/evidence_cache_keys.py`, `detection/cache_keys.py` | profile / edge-refine / enhanced profile cache key 只包含 box 与参数对象；cache adapter 归 `x5crop.cache`，`geometry` 只保留纯 profile / search / refinement 算法；layout 由 `AnalysisCache` 作用域承担，format identity 不进入底层 cache key；diagnostic cache key 必须包含实际影响搜索的 diagnostic policy。 |
