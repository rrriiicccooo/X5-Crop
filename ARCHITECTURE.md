...
- holder boundary 只有在全部最高支持 edge-adjacent paths 存在共同 position interval 时才成立，并
  ...
  clipped endpoint 权限只属于这些原始路径，且 solver 必须把其 endpoint uncertainty 收窄到共同
  interval，不能任选一条、使用交集外坐标或让派生 provenance 冒充像素观测；
...
- supporting measurement 可以被 geometry 消费；只有 measurement 反向依赖 `FRAME_GEOMETRY`
  才构成循环证明；
- 每个 physical search 只产生 canonical `PhysicalSearchOutcome`：找到可行解、完整搜索确认约束
  矛盾、必要测量不可用或 execution budget 耗尽。空候选集合本身不代表物理矛盾；只有测量
  完整且搜索完整时的无解才是 contradicted。多个 frame-size options 中任一项不可用，整个
  count 仍保持 unavailable；已找到解与预算耗尽可以并存，但不能触发 early-stop；
- assignment evaluation count 只属于 execution statistics 和 solver/build result，不进入
...
consensus、替代几何和 canonical physical-search outcome 是否已解决。Execution budget 与测量缺失
都是 typed unavailable causes，不保存在 geometry 内。Unresolved geometry 可以进入 provisional
