...
独立提出 holder-material、tonal、texture 或 canvas path。每条 measured path 保存同一
`GrayMaterialObservation` 类型的 `outer_material` 与 `inner_material`；side 和 edge adjacency 属于
path，不属于材料类型。片夹由 edge adjacency、跨截面连续性、低纹理/低梯度与可选灰度尾部支持
共同解释，不要求最亮、最暗、纯白、纯黑或零纹理。Base boundary 不假设照片一定有黑边；四边
可分别为 holder-to-film-base、holder-to-image、canvas clip 或 unresolved。
...
start/end/center、`GrayMaterialObservation`、局部 tonal evidence、允许小范围局部中断但不可跳到
...
`GrayMaterialObservation` 使用 typed `GrayIntensityTail` 表达相对当前图像分布的 low/high/midrange，
不使用布尔“像片基”标签。Film-base reference 只能由同一 gray tail 的多个不同物理位置建立。
Reference 保存当前图像的 adaptive texture limit，并在 typed 构造边界拒绝超限材料、来源与位置
类型不一致或观测数量不足的状态。Top/bottom 轨道必须来自 measured holder path，internal
separator 必须是已选 independent assignment。跨来源
共识至少各有一个观测，同来源共识至少有两个观测。每项 `FilmBaseMaterialObservation` 保存 typed
track side 或 separator boundary，重复位置不能伪装成多位置共识。完整 hard separator sequence 与
该 reference 组成 `FilmStructureEvidence`，proof path 唯一命名为 `film_structure_led`。同一 root
gray measurement 派生的 sequence boundary 与 separator 不能互相充当独立证明；单个低纹理或灰度
尾部区域保持 unresolved。片基材料不可见时保持 unavailable，不引入颜色或 135 片孔旁路。
...
- 由当前图像自适应纹理参考支持的 edge-adjacent outer material 只生成有界
...
root boundary scale 只能由 holder-to-image contact 给出 px/mm 下限；film-base reference 尚不存在，
因此不能生成上限。Candidate-local aperture evidence 建立后，holder-to-film-base contact 才能给出
短轴 px/mm 上限。
...
Candidate evidence 包括 frame coverage、film structure、photo dimensions、gray holder material、
aperture contact、candidate-local scale resolution、content alignment、partial edge safety、holder
occupancy、sequence conservation 和 evidence independence。
Evidence 只保存测量与状态，不携带固定 report 描述副本。
`AssessedCandidate` 从同一 geometry 与 film-base texture limit 确定性重建 holder material、film-base
reference、film structure 和 aperture contact identity；report restoration 不能把这些 evidence 换到
不存在的 boundary 或不同 material observation。
Candidate-local scale observations 同样必须从该 geometry 与 aperture evidence 原样重建；context
...
Holder material 与 aperture contact 只保存逐边灰度材料/path 事实；它们不因片夹极性、颜色或
calibration unavailable 形成 Gate blocker。
...
schema_revision: gray_material_sequence_resolution
```
...
selected geometry -> FinalizationPlan -> final geometry 的精确 identity，并由 decision owner 验证
