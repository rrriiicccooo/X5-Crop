...
| Gap / Separator | separator、geometry、content gap 证据如何分类和消费？ | 已完成族群分类，并按 candidate proposal 模型建立 `detection.candidate.proposal.separator` proposal / refinement / evidence 入口。 | separator 只提出或修正 gap evidence；当前 hard gap 只有 `detected / edge-pair / enhanced-detected`。 | 进行中 | 从 `separator_profile` / `find_gap` 开始逐项深审。 |
...
| Gap / separator | separator proposal | `detection/candidate/proposal/separator/proposal.py`, `geometry/gap_search.py` | normal / broad-width gap proposal 是否只提出 gap evidence；gap width、guard、peak、geometry constraint 是否符合当前 format/mode；width profile 参数归 `SeparatorPolicy.width_profile`，outer policy 只保留 separator-derived outer family/band 参数。 |
