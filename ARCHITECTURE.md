...
`ImageProfile` 是由 TIFF I/O 边界创建的 immutable typed input contract。TIFF rational、enum 和
NumPy scalar 等库特有值在 `x5crop.io` 内归一化为普通 Python scalar/tuple；calibration、cache、
report 和 export 不再解析底层 TIFF tag 形状。
