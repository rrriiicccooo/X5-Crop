...
| Gap / separator | nearby correction, enhanced separator, gap search profile planning, broad-width detected gaps | `geometry/nearby_separator.py`, `geometry/enhanced_separator.py`, `detection/gap_profiles.py`, `detection/candidate/plan/separator_width_profile.py`, `detection/evidence/separator.py` | Profile evidence must be explicit, capped when needed, and assessed through the normal candidate gate; reliable primary separator candidates may skip broad-width profile execution, and auto-count partial does not let broad-width profiles compete. |
