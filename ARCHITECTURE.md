...
| Gap / separator | robust grid model gaps | `geometry/gap_geometry.py`, `geometry/robust_grid.py`, `geometry/model_gaps.py`, `detection/candidate/build/separator_gaps.py` | grid 只能补模型证据；weak grid 不得单独获得 auto PASS；candidate build 直接调用 robust grid helper；grid 已拆成 reliable anchor selection、fit candidate generation、fit ranking、fit assessment、predicted center 和 hard-gap protection / override adjustment；`grid_model_gap` 统一生成 grid method identity；gap geometry helper 只提供几何约束，不寻找 separator；robust grid 不接收 format identity，只消费 gaps、origin/pitch、strip mode 和参数对象，并通过 `gap_methods.py` 判断 hard evidence family，方便单独审核 model gap 何时能覆盖弱 hard gap。 |
