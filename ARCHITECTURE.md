...
9. `x5crop.runtime` / `x5crop.analysis_cache` / `x5crop.geometry` /
   ...
   - 需要 format 上下文的 helper 应显式接收 format 或 policy；基础层不得自行读取
     policy registry。
   ...
   - `geometry.detection_parameters` 拥有低层 outer / separator / gap / grid 参数类型。
