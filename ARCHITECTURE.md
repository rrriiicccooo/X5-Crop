...
| `frame_sequence_candidates.py` | Candidate build state, separator bindings, physical objectives, geometry-alternative clustering, physical Pareto, representative selection, and visible-content conservation. |
| `frame_sequence_solver.py` | Remaining candidate construction, consensus, separator-assignment, boundary-role, completion, and top-level orchestration responsibilities pending their own canonical owners. |
...
lower owner; the solver consumes all four modules. No lower owner imports the solver.
...
聚类、物理 Pareto、代表解与可见内容守恒只由 `frame_sequence_candidates.py` 拥有。candidate
construction、共识、separator assignment、边界角色与补全仍由 solver 承载，等待后续独立 owner。
依赖保持单向：measurement facts → common-width → search，candidate state/ranking 作为并列低层
owner，由 solver 统一消费；低层模块不得反向导入 solver，solver 也不兼容导出已经迁移的符号。
