...
Final sequence identity also conserves separator spacing authority: every typed
separator assignment must bind the matching positive `OBSERVED` spacing at the
same boundary, and that spacing provenance must retain the assigned band's
observation identity. A geometry-hypothesis spacing cannot coexist with a hard
separator assignment even when its coordinates happen to match.

最终序列 identity 还必须守恒 separator spacing 权限：每个 typed separator assignment 必须在
同一 boundary 绑定对应的正值 `OBSERVED` spacing，且 spacing provenance 必须保留已分配 band 的
观测 identity。即使坐标碰巧一致，geometry-hypothesis spacing 也不能与 hard separator assignment
同时成立。
