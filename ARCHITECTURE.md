...
V4.9 是一次 evidence-governed policy reset。目标不是为了让更多困难样片
...
- `x5crop.detection.final.decision` 统一执行 PASS / REVIEW 决策。
...
10. `x5crop.detection`
   ...
   - `outer/` 只提出 outer proposal：`base.py`、`content.py`、
     `edge_anchor.py`、`separator_first.py`、`separator_geometry.py`、
     `dark_band.py`、`alignment.py` 和 retry 子模块分别拥有各自策略。
   ...
   - `candidate/` 只负责候选生命周期：count plan、source build、candidate run、
     candidate-level gates、candidate decision、scoring、selection、partial holder
     和 fallback。
   - `modes/` 承接专用 detector，例如 135-dual lane 和 unsupported review-only。
   - `final/decision.py` 在 finalization 中统一执行 V4.9 conservative PASS/REVIEW。
   - `final/geometry.py` 负责输出前最终几何调整，包括 detection/output bleed、
...
| 10 Detection：`x5crop.detection` | 已人工审核并清理 | 彻底干净 | Detection 已拆为 `outer/`、`evidence/`、`candidate/`、`modes/` 和 `final/` 子包；旧平铺 `outer.py`、`outer_retry.py`、`candidate_run.py`、`content.py`、`diagnostics.py`、`finalizer.py` 等 compatibility surface 已移除；package `__init__` 只保留 marker。 |
...
| Runtime cache / foundation | `runtime.py`、`analysis_cache.py`、`geometry/*`、`image/*`、`io/*` | 彻底干净。`runtime.py` 只定义 shared cache containers，`analysis_cache.py` 只构建 per-image cache；geometry / image / io 保持低层能力边界，不承载 candidate、gate、PASS/REVIEW 或 output bleed 语义。 | 需要新增共享 cache 字段时先确认它是 detection/debug 共用数据，不把 final decision 或 report rendering 状态塞进 cache。 |
...
- `x5crop.policies.decision_contract` owns the public decision contract:
  ...
  DecisionDiagnosticsPolicy.
- `x5crop.detection.final.decision` applies the unified PASS / REVIEW decision.
...
10. `x5crop.detection`
   ...
   - `outer/` only proposes outer boxes. `base.py`, `content.py`,
     `edge_anchor.py`, `separator_first.py`, `separator_geometry.py`,
     `dark_band.py`, `alignment.py`, and retry modules own their specific
   ...
   - `candidate/` owns the candidate lifecycle: count planning, source build,
     candidate run, candidate-level gates, candidate decision, scoring,
     selection, partial holder, and fallback.
   - `modes/` owns dedicated detectors, such as 135-dual lane and unsupported
     review-only paths.
   - `final/decision.py` applies conservative V4.9 PASS/REVIEW rules during finalization.
   - `final/geometry.py` owns final pre-output geometry adjustment, including
...
| 9 Runtime Cache / Geometry / Image / IO | Deep-reviewed and cleaned | Fully clean | Lower-level capability boundaries are clear: runtime / analysis_cache only own shared cache containers and builders, geometry owns box/gap/frame helpers, image owns grayscale/deskew/pixel transforms, and io owns TIFF profile handling; no candidate, gate, PASS/REVIEW, or output bleed semantics belong here. |
