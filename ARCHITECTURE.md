...
| Gap / separator | gap method consumers | `constants.py`, `detection/candidate/assessment/*`, `detection/decision/pass_review.py`, `detection/evidence/*`, `geometry/frame_fit.py`, `debug/gaps.py` | assessment、decision、risk/read-only diagnostics、frame edge fitting 和 debug gap overlay 只消费 `GAP_*` 常量与 `HARD_GAP_METHODS` / `MODEL_GAP_METHODS` 集合；不在消费层重新手写 separator / model method 字符串；candidate scoring 通过 `GapMethodEvidenceSummary` 明确区分 raw gap methods，separator gate detail 消费通过 `SeparatorGateDetailSummary` 明确区分 expected、hard separator、grid model 和 equal model，避免把 grid model support 误读成真实 detected hard gap。 |
...
| Candidate scoring | separator support score | `detection/candidate/assessment/scoring.py` | hard/grid/equal 的信用是否符合 hard > model 的原则；raw gap scoring 使用 `GapMethodEvidenceSummary`，gate-detail scoring 使用 `SeparatorGateDetailSummary`；`detected_gaps` 是兼容 report 字段，内部使用 `separator_support_gaps = hard_separator_gaps + grid_model_gaps`，不得把 grid model gap 命名为 hard detected evidence。 |
...
| Scoring | base confidence, geometry/content/separator scores, joint score, hard-full floor | `detection/candidate/assessment/scoring.py`, `detection/candidate/assessment/candidate.py` | Scores support gates; they do not replace separator/content/geometry requirements. `GapMethodEvidenceSummary` separates raw gap method evidence before base scoring, and `SeparatorGateDetailSummary` separates expected, hard separator, grid model, and equal model counts before gate-detail scoring, so model support is not presented as hard detected evidence. |
