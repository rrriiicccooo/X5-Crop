...
| `frame_sequence_candidates.py` | Candidate build state, separator bindings, slot-topology invariants, physical objectives, geometry-alternative clustering, physical Pareto, representative selection, and visible-content conservation. |
...
| `frame_sequence_solver.py` | Remaining candidate construction, separator-assignment, boundary-role, blank/occlusion/completion, and top-level orchestration responsibilities pending their own canonical owners. |

Dependency direction is one way: common-width resolution consumes measurement
facts; search consumes both lower owners; candidate state/ranking is an independent
lower owner; consensus consumes candidate state; the solver consumes all five modules.
...
与 external safety envelope 只由 `frame_sequence_consensus.py` 拥有。candidate construction、
separator assignment、边界角色与 blank/occlusion/completion 仍由 solver 承载，等待后续独立
owner。依赖保持单向：measurement facts → common-width → search，candidate state/ranking 作为并列
低层 owner 并由 consensus 消费，solver 统一消费全部低层 owner；低层模块不得反向导入 solver，
solver 也不兼容导出已经迁移的符号。
