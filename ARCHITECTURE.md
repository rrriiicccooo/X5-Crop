...
| Gap / separator | separator profile | `geometry/separator_profile.py`, `geometry/edge_refine_profile.py` | 基础 separator profile 只生成一维 separator signal；edge-refine profile 已拆为 edge-pair correction 专用证据。中段采样、分段极端亮/暗、uniform soft score、列向梯度、edge peak search 分别可审。 |
