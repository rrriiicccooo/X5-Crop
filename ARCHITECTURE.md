...
| Gate | separator gate | `detection/candidate/assessment/gates.py` | gate profile 是否与 format/mode policy 一致；`SeparatorGateEvidence` 统一汇总 hard / model / width / leading-grid evidence，`SeparatorGateAssessment` 统一负责 single-frame / threshold / leading-grid / profile dispatch，detail serializer 只输出既有 report keys。 |
