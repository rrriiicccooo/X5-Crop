...
| `policies.decision` | final PASS / REVIEW decision contract；消费已装配的 decision evidence parameters。 |
...
reporting 只读取并描述它。
decision evidence parameters 由 central parameter registry 根据 physical traits 构建，不使用
format-id override 表；format 名称只作为 `FormatPhysicalSpec` 查询入口，实际差异来自 family、
...
grouped parameters. `policies.decision` owns the final decision contract and
consumes assembled decision evidence parameters.
...
The central parameter registry derives decision evidence parameters from physical
traits without format-id override tables. A format name is only a
`FormatPhysicalSpec` lookup key; behavior differences come from physical facts such
