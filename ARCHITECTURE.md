...
| Outer proposal | grid-refined outer | `detection/candidate/proposal/outer/grid_refine.py`, `detection/candidate/build/separator_gaps.py` | grid-derived outer box 计算属于 outer proposal helper；separator gap lifecycle 只消费 grid detail 并在允许时重新生成 gaps，不拥有 outer box 修正规则。 |
...
| Gap / separator | separator proposal | `detection/candidate/proposal/separator/proposal.py`, `detection/candidate/proposal/separator/model.py`, `geometry/model_gaps.py`, `geometry/gap_search.py`, `geometry/gap_geometry.py`, `geometry/separator_band.py`, `geometry/separator_width_profile.py` | normal / broad-width gap proposal 是否只提出 gap evidence；`gap_search.py` 只负责从 profile 中找 standard detected/equal gaps，window、width limits、thresholds、band expansion、band evidence measurement、prominence support、width-profile support、detected candidate collection、best-candidate selection 和 equal fallback 已拆成独立 helper，且关键阈值归 `GapSearchParameters`；显式 equal / grid / content model gap 生成归 `geometry/model_gaps.py`，profile 等分模型归 `separator/model.py`，避免在 build / safety / refinement / content candidate 路径手写 method 字符串；gap 几何约束、width CV 和局部几何误差归 `gap_geometry.py`，供 grid / nearby refinement 消费；普通 separator-derived outer bands 与 broad-width bands 共用 `SeparatorBand` 物理表示；width profile 纯数学归 `geometry/separator_width_profile.py`，参数归 `SeparatorPolicy.width_profile`，outer policy 只保留 separator-derived outer family/band 参数；proposal helper 只接收 gap search 或 width profile 参数，不接收完整 `DetectionPolicy`。 |
...
| Gap / separator | separator gap lifecycle in candidate build | `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/detection.py` | `build_detection_for_outer` 是否只消费 separator gap lifecycle 结果去生成 frames、score 和 detail；separator gap lifecycle helper 负责 origin/pitch、standard/broad-width gaps、edge-pair、grid、enhanced 和 nearby refinement，但不 PASS / REVIEW；grid-derived outer box 计算委托给 outer proposal helper。 |
...
| Outer proposal | grid-refined outer | `detection/candidate/proposal/outer/grid_refine.py`, `detection/candidate/build/separator_gaps.py` | Grid-derived outer box calculation belongs to an outer proposal helper; the separator gap lifecycle consumes grid detail and may rebuild gaps, but it does not own the outer-box adjustment rule. |
...
| Gap / separator | profile, cache, proposal, hard trust, refinement, grid model | `geometry/separator_*`, `geometry/model_gaps.py`, `geometry/edge_refine_profile.py`, `geometry/gap_search.py`, `geometry/gap_geometry.py`, `geometry/gap_trust.py`, `detection/candidate/proposal/separator/*` | Hard evidence must stay stronger than model/equal/grid/content evidence, and cache keys must include policy-relevant context. Separator proposal/refinement only creates or adjusts gap evidence; it does not PASS / REVIEW. Base separator profile generation, explicit equal / grid / content model-gap generation, edge-pair edge-refine profile generation, profile cache ownership, gap-search band evidence measurement, broad-width candidate selection, width evidence requirement detail, typed separator bands, gap geometry metrics, edge-pair replacement, enhanced promotion, nearby replacement, grid fit / adjustment, and runtime / diagnostic hard-gap trust classifiers are split into small helpers for review. Candidate build owns cached edge/background profile retrieval and separator width evidence creation; geometry edge-pair helpers consume profile arrays only. Enhanced min score and nearby width-CV slack are explicit policy/report fields. Width profile math lives in `geometry/separator_width_profile.py`; width profile parameters live under `SeparatorPolicy.width_profile`; outer policy only owns separator-derived outer family/band parameters. |
...
| Gap / separator | separator gap lifecycle in candidate build | `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/detection.py` | `build_detection_for_outer` consumes separator gap lifecycle output for frame building, scoring, and detail writing. The lifecycle helper owns origin/pitch, standard/broad-width gaps, edge-pair, grid, enhanced, and nearby refinement, but not PASS / REVIEW; grid-derived outer-box calculation is delegated to the outer proposal helper. |
