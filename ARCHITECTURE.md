...
| Gap / separator | gap method consumers | `constants.py`, `detection/candidate/assessment/*`, `detection/decision/pass_review.py`, `detection/evidence/*`, `geometry/frame_fit.py` | assessment、decision、risk/read-only diagnostics 和 frame edge fitting 只消费 `GAP_*` 常量与 `HARD_GAP_METHODS` / `MODEL_GAP_METHODS` 集合；不在消费层重新手写 separator / model method 字符串。 |
