...
| Outer proposal | separator-derived outer | `detection/outer/proposal/separator.py`, `detection/evidence/separator_bands.py` | separator-derived outer 是否按 `outer_scope`（local / full-width）与 `gap_search_profile`（standard / broad_width）组合生成候选；broad_width 是 separator gap/search/evidence profile，不是 outer variant；标准 strip 的 full 全部 eligible，partial 只有显式 count 时 eligible extension profiles；candidate execution budget 可在可靠 primary 结果后跳过 extension 执行。 |
...
| Candidate extension | outer correction candidate extension | `detection/pipeline.py`, `detection/candidate/outer_correction_candidates.py` | correction 顺序固定为 geometry consistency 再 content containment；candidate plan 记录 eligible / skipped / attempted family；可靠 selection 且 outer alignment ok 时可跳过 correction 计算，并写出 execution budget action / reason；pipeline 只把 reassessed corrected outer 追加回候选池，不能绕过最终 decision / gate。 |
...
| Gap / separator | gap search profile planning | `detection/gap_profiles.py`, `detection/candidate/separator_width_profile_plan.py`, `detection/candidate/sources.py` | `standard` 与 `broad_width` 是否作为同一候选计划里的 gap search profiles；full 和显式 count partial 可进入统一 assessment，但 primary separator 已可靠时可以不执行 broad-width profile；auto-count partial 不让 broad-width profile 抢候选。 |
...
| Candidate plan | gap search profiles | `detection/gap_profiles.py`, `candidate/separator_width_profile_plan.py`, `candidate/sources.py`, `candidate/reliability.py` | `standard` 和 `broad_width` 是否作为同一候选计划的 eligible profiles；execution budget 可在 reliable primary 后跳过更贵的 profile 执行，并在 detail 中写出 action / reason / stage；不得退回失败补救式 retry。 |
...
| Candidate extension | outer correction candidate extension | `detection/pipeline.py`, `detection/candidate/outer_correction_candidates.py` | The pipeline appends reassessed corrected outer candidates back into the candidate pool before selection; candidate detail records eligible, skipped, attempted correction families, and execution budget action / reason. Reliable selected candidates may skip correction computation only when outer alignment is also ok. |
...
| Gap / separator | nearby correction, enhanced separator, gap search profile planning, broad-width detected gaps | `geometry/nearby_separator.py`, `geometry/enhanced_separator.py`, `detection/gap_profiles.py`, `detection/candidate/separator_width_profile_plan.py`, `detection/evidence/separator.py` | Profile evidence must be explicit, capped when needed, and assessed through the normal candidate gate; reliable primary separator candidates may skip broad-width profile execution, and auto-count partial does not let broad-width profiles compete. |
...
| Candidate | count/offset, source orchestration, build, frame fit | `detection/candidate/counts.py`, `detection/candidate/run.py`, `detection/candidate/sources.py`, `detection/gap_profiles.py`, `detection/candidate/reliability.py`, `detection/candidate/build.py`, `geometry/frame_fit.py` | Candidate lifecycle must keep all intermediate evidence in `Detection.detail`; primary separator candidates are assessed before extension profiles / outer scope/profile combinations execute. |
...
| Candidate plan | gap search profiles, safety candidate, broad-width profile selection, partial stop | `detection/candidate/run.py`, `detection/candidate/sources.py`, `detection/gap_profiles.py`, `detection/candidate/reliability.py`, `detection/candidate/separator_width_profile_plan.py`, `detection/candidate/separator_width_profile_selection.py` | Candidate plan profiles and families are declared together, but execution budget may stop after a reliable primary candidate and reports action / reason / stage; no source can bypass hard evidence. |
