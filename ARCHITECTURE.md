...
7. `x5crop.policies`
   ...
   - `runtime_policy.py` 定义 runtime `DetectionPolicy` contract；runtime
     ...
     `runtime_final.py`、`runtime_diagnostics.py` 分组；低层 geometry config
...
9. `x5crop.runtime` / `x5crop.analysis_cache` / `x5crop.geometry` /
   ...
   - `geometry.detection_parameters` 拥有低层 outer / separator / gap / grid
     config 类型；这些类型不使用 runtime policy 命名。
...
10. `x5crop.detection`
   ...
   - `candidate/` 只负责候选生命周期：count plan、source build、candidate run、
     candidate-level gates、candidate_decision、scoring、selection、partial holder
...
11. `x5crop.output_geometry` / `x5crop.analysis_reuse` / `x5crop.export` / `x5crop.result_builder` /
    `x5crop.report_schema` / `x5crop.report_outputs` / `x5crop.debug`
   - `output_geometry.py` 拥有 detection/output bleed config、overlap output
     bleed 判断和 cached output bleed 重放；fresh finalization 和 cached analysis
     reuse 共同消费它。
...
| 7 Policy：`x5crop.policies` | 已人工审核并清理 | 彻底干净 | runtime policy types、factory builders、parameter modules、format registry 和 parameter registry 已按职责分组；旧 `parameters.py` / `parameter_types.py` 兼容 re-export 层已删除。`runtime_policy.py` 不再转出口低层 geometry config；`parameter_aggregate.py` 只承担 flat `FormatParameters` aggregate 一个职责。 |
...
| 9 Runtime Cache / Geometry / Image / IO | 已深度审核并清理 | 彻底干净 | 基础能力边界清楚：runtime / analysis_cache 只拥有共享缓存容器和构建器，geometry 返回低层 box / gap / frame helper，image 负责灰度/deskew/像素变换，io 负责 TIFF profile；不拥有 candidate、gate、PASS/REVIEW 或 output bleed 语义。 |
| 10 Detection：`x5crop.detection` | 已人工审核并清理 | 彻底干净 | Detection 已拆为 `outer/`、`evidence/`、`candidate/`、`modes/` 和 `final/` 子包；旧平铺 `outer.py`、`outer_retry.py`、`candidate_run.py`、`content.py`、`diagnostics.py`、`finalizer.py` 等 compatibility surface 已移除；package `__init__` 只保留 marker；output bleed 共享逻辑已移到 `output_geometry.py`。 |
| 11 Output / Report / Debug：`output_geometry`、`analysis_reuse`、`export`、`result_builder`、`report_schema`、`report_sections`、`report_outputs`、`debug` | 已人工审核并清理 | 彻底干净 | `output_geometry.py` 拥有 output bleed 共享逻辑，避免 cached reuse 依赖 detection final 内部；Debug 已拆为 canvas、gap overlays、panels、status 和 writer；`report_schema` 只组装稳定 schema，candidate/gate section builder 归属 `report_sections.py`。空的 `x5crop.diagnostics` 占位包已删除。 |
...
| Runtime cache / foundation | `runtime.py`、`analysis_cache.py`、`geometry/*`、`image/*`、`io/*` | 彻底干净。`runtime.py` 只定义 shared cache containers，`analysis_cache.py` 只构建 per-image cache；geometry / image / io 保持低层能力边界，低层参数使用 config 命名，不承载 candidate、gate、PASS/REVIEW 或 output bleed 语义。 | 需要新增共享 cache 字段时先确认它是 detection/debug 共用数据，不把 final decision 或 report rendering 状态塞进 cache。 |
| Output / Report / Debug | `output_geometry.py`、`analysis_reuse.py`、`result_builder.py`、`report_schema.py`、`report_sections.py`、`report_outputs.py`、`export/*`、`debug/*` | 彻底干净。`output_geometry.py` 是 fresh finalization 与 cached reuse 共享的 output bleed helper；`report_schema.py` 只做稳定 schema assembly；candidate table、selected candidate 和 gate records 在 `report_sections.py`；Debug 已拆成 canvas、gap overlays、panels、status、writer、outputs，空的 `x5crop.diagnostics` 包已删除。 | Detection 内部只读诊断已改归 `x5crop.detection.evidence.read_only` / `gap_diagnostics` / `risk`，不再使用容易和 output/debug 混淆的 `detection/diagnostics.py`。 |
...
7. `x5crop.policies`
   ...
   - `runtime_policy.py` defines the runtime `DetectionPolicy` contract; runtime
     ...
     `runtime_separator.py`, `runtime_content.py`, `runtime_candidate.py`,
     `runtime_final.py`, and `runtime_diagnostics.py`; low-level geometry config
...
9. `x5crop.runtime` / `x5crop.analysis_cache` / `x5crop.geometry` /
   ...
   - `geometry.detection_parameters` owns low-level outer / separator / gap /
     grid config types; these types do not use runtime policy naming.
...
10. `x5crop.detection`
   ...
   - `candidate/` owns the candidate lifecycle: count planning, source build,
     candidate run, candidate-level gates, candidate_decision, scoring,
...
11. `x5crop.output_geometry` / `x5crop.analysis_cache` / `x5crop.analysis_reuse` / `x5crop.export` /
    `x5crop.result_builder` / `x5crop.report_schema` / `x5crop.report_outputs` /
    `x5crop.debug`
   - `output_geometry.py` owns detection/output bleed config, overlap output
     bleed checks, and cached output bleed replay; fresh finalization and cached
     analysis reuse both consume it.
...
| 7 Policy: `x5crop.policies` | Reviewed and cleaned | Fully clean | Runtime policy types, factory builders, parameter modules, format registry, and parameter registry are grouped by responsibility; the old `parameters.py` / `parameter_types.py` compatibility re-export layers are removed. `runtime_policy.py` no longer re-exports low-level geometry config types. `parameter_aggregate.py` owns only flat defaults, while `parameter_views.py` owns derived parameter views. |
...
| 9 Runtime Cache / Geometry / Image / IO | Deep-reviewed and cleaned | Fully clean | Lower-level capability boundaries are clear: runtime / analysis_cache only own shared cache containers and builders, geometry owns box/gap/frame helpers plus low-level detection parameter types, image owns grayscale/deskew/pixel transforms plus deskew parameters, and io owns TIFF profile handling; no candidate, gate, PASS/REVIEW, or output bleed semantics belong here. |
| 10 Detection: `x5crop.detection` | Reviewed and cleaned | Fully clean | Detection is split into `outer/`, `evidence/`, `candidate/`, `modes/`, and `final/` subpackages. The old flat `outer.py`, `outer_retry.py`, `candidate_run.py`, `content.py`, `diagnostics.py`, `finalizer.py`, and related compatibility surfaces are removed; package `__init__` files are markers only; shared output bleed logic lives in `output_geometry.py`. |
| 11 Output / Report / Debug: `output_geometry`, `analysis_reuse`, `export`, `result_builder`, `report_schema`, `report_sections`, `report_outputs`, `debug` | Reviewed and cleaned | Fully clean | `output_geometry.py` owns shared output bleed logic so cached reuse does not depend on detection final internals. Debug is split into canvas, gap overlays, panels, status, and writer modules. `report_schema` only assembles the stable schema, candidate/gate section builders live in `report_sections.py`, and the empty `x5crop.diagnostics` placeholder package has been removed. |
...
| Runtime cache / foundation | `runtime.py`, `analysis_cache.py`, `geometry/*`, `image/*`, `io/*` | Fully clean. `runtime.py` only defines shared cache containers, `analysis_cache.py` only builds per-image cache, and geometry / image / io keep low-level capability boundaries with config naming and without candidate, gate, PASS/REVIEW, or output bleed semantics. | Before adding a shared cache field, confirm it is data shared by detection/debug and not final decision or report-rendering state. |
| Output / Report / Debug | `output_geometry.py`, `analysis_reuse.py`, `result_builder.py`, `report_schema.py`, `report_sections.py`, `report_outputs.py`, `export/*`, `debug/*` | Fully clean. `output_geometry.py` is the shared output bleed helper consumed by fresh finalization and cached reuse. `report_schema.py` only assembles the stable schema. Candidate table, selected candidate, and gate records live in `report_sections.py`. Debug is split into canvas, gap overlays, panels, status, writer, and outputs, and the empty `x5crop.diagnostics` package is removed. | Detection-internal read-only diagnostics now live in `x5crop.detection.evidence.read_only` / `gap_diagnostics` / `risk`, avoiding confusion with the output/debug layer. |
