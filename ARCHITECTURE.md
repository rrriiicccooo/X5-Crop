...
| Gap / separator | separator gap lifecycle in candidate build | `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/detection.py` | `build_detection_for_outer` 是否只消费 separator gap lifecycle 结果去生成 frames、score 和 detail；separator lifecycle 已拆成 primary gap build（origin/pitch、standard/broad-width gaps、edge-pair、grid）和 late refinements（enhanced、nearby）；initial gap proposal 用 `InitialSeparatorGapResult` 承接 standard、broad-width selection 和 detected-geometry model source selection，不再裸 tuple 传递；`standard_gap_search` detail 记录 `selected_gap_source` 而不是 override 语义；primary refinement 用 `PrimarySeparatorRefinementResult` 承接 edge-pair 与 grid 输出；late refinement 用 `LateSeparatorRefinementResult` 承接 enhanced promotion、nearby correction、最终 gaps 和 pre-nearby gaps；late refinement 只消费 `analysis_mode`，不持有完整 `RuntimeConfig`；grid-derived outer refine 由 candidate build 在两段之间重新 build，不由 separator lifecycle 决定 PASS / REVIEW 或 outer 修正规则。 |
