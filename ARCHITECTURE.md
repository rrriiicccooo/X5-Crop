...
| Gap / Separator | separator、geometry、content gap 证据如何分类和消费？ | 已完成族群分类，并按 candidate proposal 模型建立 `detection.candidate.proposal.separator` proposal / refinement / evidence 入口；candidate build 层已抽出 separator gap lifecycle helper；普通 separator profile / gap search、edge-pair / enhanced / nearby refinement、robust grid refinement 和 hard-gap trust 分类已拆成可审核 helper；普通 detected gap search 已把 band evidence、acceptance 和 ranking 分开；broad-width detected gap search 已把 run、width acceptance、candidate scoring 和 ranking 分开；enhanced gap promotion / nearby 的关键阈值已进入 policy detail；assessment / decision / diagnostics / frame edge fitting 消费层已统一使用 gap method 常量；ordinary gap search 与 broad-width gap proposal 的关键搜索参数已外显；profile / edge-refine / enhanced profile cache 已去掉 format identity；nearby correction 不再在 separator build 中调用 scoring。 | separator 只提出或修正 gap evidence；当前 hard gap 只有 `detected / edge-pair / enhanced-detected`；model gap 为 `grid / equal / content`；outer -> gaps 与 gaps -> frames 的职责已拆开；nearby 修正前后 gap lifecycle 由 candidate build 交给 detection build / assessment 消费；profile signal、gap search acceptance、gap search candidate ranking、broad-width run acceptance、edge-pair replacement、enhanced gap promotion、nearby replacement、grid fit / adjustment、runtime / diagnostic hard-gap trust、gap method 消费 vocabulary、ordinary gap search 参数、broad-width search 参数和 cache key ownership 可分段审核。 | 进行中 | 继续审 separator refinement 阈值策略是否还能合并、收窄或推广。 |
...
| Gap / separator | separator proposal | `detection/candidate/proposal/separator/proposal.py`, `detection/candidate/proposal/separator/model.py`, `geometry/model_gaps.py`, `geometry/gap_search.py`, `geometry/gap_geometry.py`, `geometry/separator_band.py`, `geometry/separator_width_profile.py` | normal / broad-width gap proposal 是否只提出 gap evidence；`gap_search.py` 只负责从 profile 中找 standard detected hard gap，window、width limits、thresholds、band expansion、band evidence measurement、detected acceptance、detected candidate ranking 和 best-candidate selection 已拆成独立 helper，且关键阈值归 `GapSearchParameters`；acceptance 只判断 prominence 与 width-profile support 是否足够，ranking 只比较 distance / quality / score，不混入 PASS / REVIEW 语义；`GapSearchContext` 收束单个 expected gap 的 local profile、threshold、width limits 和 ranking context；显式 equal / grid / content model gap 生成归 `geometry/model_gaps.py`，standard gap 找不到 detected 时由 `separator/proposal.py` 调用 `separator/model.py` 生成 equal fallback，避免 hard search 和 model evidence 混在一起；gap 几何约束、width CV 和局部几何误差归 `gap_geometry.py`，供 grid / nearby refinement 消费；普通 separator-derived outer bands 与 broad-width bands 共用 `SeparatorBand` 物理表示；width profile 纯数学归 `geometry/separator_width_profile.py`，搜索参数归 `SeparatorPolicy.width_profile_search`，启用 / 候选 / selection 策略归 `SeparatorPolicy.width_profile`，outer policy 只保留 separator-derived outer family/band 参数；proposal helper 只接收 gap search 或 width profile 参数，不接收完整 `DetectionPolicy`。 |
| Gap / separator | hard-gap trust | `geometry/gap_trust.py`, `detection/evidence/gap_diagnostics.py` | hard gap 可信度是否能区分真实片间空隙、frame border、content edge；像素 signal、runtime trust classifier 和 diagnostic trust classifier 已集中在 `geometry/gap_trust.py`，diagnostics 只负责采样记录，不重复分类规则。 |
| Gap / separator | separator refinement | `geometry/edge_refine_profile.py`, `geometry/edge_pairs.py`, `geometry/nearby_separator.py`, `geometry/enhanced_separator.py`, `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/detection.py` | edge-pair / nearby / enhanced 只能修正或补充 gap evidence；不得直接 PASS；candidate build 直接调用 geometry refinement helpers，不保留只做别名转发的 proposal refinement facade；edge-pair 已明确为 gap correction，detail key 为 `edge_pair_correction`，并已拆成 edge-refine profile、profile cache owner、search limits、typed candidate generation、best-pair selection 和 replacement eligibility；policy assembly 会把 format preset 转成 `EdgePairParameters`，`geometry/edge_pairs.py` 只消费 profile arrays 和基础参数对象；enhanced 已明确为 gap promotion：detected gap validation 后把 model/equal gap 提升为 `enhanced-detected`，detail key 为 `enhanced_gap_promotion`；nearby 已拆成 search context、candidate ranking、stronger test 和 geometry acceptance，并只把 correction detail 与 pre-correction gaps 交给 detection build；confidence cap / scoring 保留在 detection build / assessment 消费层；`SeparatorPolicy.hard_methods` / `model_methods` 使用真实 gap method vocabulary，并在 policy/report detail 中显式可见。 |
| Gap / separator | robust grid model gaps | `geometry/gap_geometry.py`, `geometry/robust_grid.py`, `geometry/model_gaps.py`, `detection/candidate/build/separator_gaps.py` | grid 只能补模型证据；weak grid 不得单独获得 auto PASS；candidate build 直接调用 robust grid helper；grid 已拆成 reliable anchor selection、fit candidate generation、fit ranking、fit assessment、predicted center 和 hard-gap protection / override adjustment；`grid_model_gap` 统一生成 grid method identity；gap geometry helper 只提供几何约束，不寻找 separator；robust grid 不接收 format identity，只消费 gaps、origin/pitch、strip mode 和参数对象，方便单独审核 model gap 何时能覆盖弱 hard gap。 |
| Gap / separator | gap method consumers | `constants.py`, `detection/evidence/separator_summary.py`, `detection/candidate/assessment/*`, `detection/candidate/selection/*`, `detection/decision/pass_review.py`, `detection/evidence/*`, `geometry/frame_fit.py`, `debug/gaps.py` | assessment、selection、decision、risk/read-only diagnostics、frame edge fitting 和 debug gap overlay 只消费 `GAP_*` 常量与 `HARD_GAP_METHODS` / `MODEL_GAP_METHODS` 集合；不在消费层重新手写 separator / model method 字符串；raw gap 消费通过 `GapMethodEvidenceSummary` 明确区分 hard separator、grid model、equal model 和 separator support，width-profile selection 不再把 non-equal gap 当成 hard gap；separator gate detail 消费通过 `SeparatorGateDetailSummary` 明确区分 expected、hard separator、grid model、equal model 和 content model，避免把 grid/content model support 误读成真实 detected hard gap。 |
...
| Gate | separator gate | `detection/candidate/assessment/gates.py`, `policies/separator_gate_profiles.py`, `policies/assembly/separator.py` | gate profile 是否与 format/mode policy 一致；gate profile vocabulary 集中在中性 policy vocabulary 模块，assembly 会校验未知 profile；`assess_separator_gate()` 是 active entry，`SeparatorGateEvidence` 统一汇总 hard / model / width / leading-grid evidence，`SeparatorGateAssessment` 统一负责 single-frame / threshold / leading-grid / profile dispatch，detail serializer 只输出既有 report keys。 |
...
| Gap / separator | profile, cache, proposal, hard trust, refinement, grid model | `geometry/separator_*`, `geometry/model_gaps.py`, `geometry/edge_refine_profile.py`, `geometry/gap_search.py`, `geometry/gap_geometry.py`, `geometry/gap_trust.py`, `detection/candidate/proposal/separator/*`, `detection/candidate/build/separator_gaps.py` | Hard evidence must stay stronger than model/equal/grid/content evidence, and cache keys must include policy-relevant context. Separator proposal creates gap evidence; geometry refinement helpers adjust or promote it; neither can PASS / REVIEW. Base separator profile generation, explicit `SeparatorProfileSignals`, combined profile scoring, explicit equal / grid / content model-gap generation, edge-pair edge-refine profile generation, profile cache ownership, hard-only gap-search band evidence measurement, broad-width candidate selection, width evidence requirement detail, typed separator bands, gap geometry metrics, edge-pair replacement, enhanced promotion, nearby replacement, grid fit candidate / assessment, grid adjustment, and runtime / diagnostic hard-gap trust classifiers are split into small helpers for review. Candidate build directly calls the geometry refinement helpers, owns cached edge/background profile retrieval and separator width evidence creation, and no longer keeps a proposal refinement facade that only re-exports functions. Edge-pair preset policy is assembled into `EdgePairParameters` before geometry sees it. Enhanced min score and nearby width-CV slack are explicit policy/report fields. Width profile math lives in `geometry/separator_width_profile.py`; search parameters live under `SeparatorPolicy.width_profile_search`, while activation / candidate / selection policy lives under `SeparatorPolicy.width_profile`; outer policy only owns separator-derived outer family/band parameters. |
...
| Scoring | base confidence, geometry/content/separator scores, joint score, hard-full floor | `detection/candidate/assessment/scoring.py`, `detection/candidate/assessment/candidate.py`, `detection/evidence/separator_summary.py` | Scores support gates; they do not replace separator/content/geometry requirements. `GapMethodEvidenceSummary` separates raw gap method evidence before base scoring and width-profile selection, and `SeparatorGateDetailSummary` separates expected, hard separator, grid model, equal model, and content model counts before gate-detail scoring / decision consumption, so model support is not presented as hard detected evidence. |
| Gate | separator gate profiles and geometry support | `detection/candidate/assessment/gates.py`, `detection/candidate/assessment/scoring.py`, `policies/separator_gate_profiles.py`, `policies/assembly/separator.py` | Separator gate profile names are a single neutral policy vocabulary and are validated during policy assembly; unknown profiles cannot silently fall back to strict mode. `assess_separator_gate()` is the active gate entry. `SeparatorGateEvidence` summarizes hard/model/width/leading-grid evidence before profile evaluation, and `SeparatorGateAssessment` owns single-frame, threshold, leading-grid, and gate-profile dispatch. `min_hard_with_equal_cap`, `all_internal_gaps_hard`, and `geometry_support` must match format/mode policy; full/default-count supplemental broad-width and edge-pair checks stay separate from the main hard-gap profile, and leading-grid failure is split into explicit leading-score, late-hard-gap, and enhanced-promotion guard helpers. |
