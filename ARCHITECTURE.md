...
| 子包 | 职责 |
|---|---|
| `policies.formats` | format-specific parameter overrides；不得声明 gate、detector、diagnostics 或 runtime preset。 |
...
| `policies.assembly` | 从 format id、物理 facts 和参数覆盖组装 runtime policy preset。 |
...
| Policy activation | format parameter overrides | `policies/formats/format_*.py` | format 文件是否只声明物理/搜索参数覆盖；不得直接声明 gate profile、detector kind、review-only、diagnostics 或 frame-fit runtime preset。 |
| Policy activation | runtime policy assembly | `policies/assembly/*`, `policies/runtime/*`, `policies/parameters/*` | assembly 是否从 format id、物理 facts 和参数覆盖生成 runtime preset；默认字段不得让报告误以为逻辑已 active。 |
...
| Gap / separator | separator refinement | `geometry/edge_refine_profile.py`, `geometry/edge_pairs.py`, `geometry/nearby_separator.py`, `geometry/enhanced_separator.py`, `geometry/gap_refinement_detail.py`, `policies/runtime/separator.py`, `policies/assembly/separator.py`, `detection/candidate/build/separator_refinements.py`, `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/detection.py` | edge-pair / nearby / enhanced 只能修正或补充 gap evidence；不得直接 PASS；candidate build 直接调用 geometry refinement helpers，不保留只做别名转发的 proposal refinement facade；`SeparatorPolicy.refinement` 用统一 `SeparatorRefinementPolicy` 声明 edge-pair、enhanced promotion 和 nearby refinement family 的 mode、phase、strip modes、partial 显式 count 要求、target gap methods 与 analysis mode 约束；standard strip 的 full 全部 eligible，partial 只有显式 count 才 eligible，dual-lane / review-only 不进入普通 refinement；`separator_refinements.py` 承接 edge-pair、robust-grid、enhanced 和 nearby refinement wrappers，`separator_gaps.py` 只编排 initial source selection、primary build 和 late refinement attachment；candidate build 用 `PrimarySeparatorRefinementResult` 承接 edge-pair correction 与 robust-grid detail，用统一 `GapRefinementResult` 承接 edge-pair、enhanced 和 nearby wrapper 输出，detail 继续写入 report key，并统一补齐 `family`、`phase`、`eligible`、`skipped_reason`、accepted / rejected count 和 family policy visibility；edge-pair 已明确为 gap refinement，detail key 为 `edge_pair_correction`，并已拆成 edge-refine profile、profile cache owner、`EdgePairSearchLimits`、typed candidate generation、best-pair selection、`edge_pair_gap_from_candidate`、replacement assessment 和 batch refinement result；edge-pair detail 现在记录 search limits、candidate count、ranked selected candidate 和 replacement assessment，accepted / rejected replacement 都记录 reason、delta、shift / quality limit，方便人工审核是否过度替换；policy assembly 通过集中 runtime preset 生成 `EdgePairParameters`，`geometry/edge_pairs.py` 只消费 profile arrays 和基础参数对象；enhanced 已明确为 gap promotion：detected gap validation 后把 geometry model gap 提升为 `enhanced-detected`，detail key 为 `enhanced_gap_promotion`，active search 入口只保留 `find_enhanced_gap_with_detail()`，不保留无 detail / bool wrapper；`EnhancedGapSearchResult`、`EnhancedGapPromotionResult` 和 `EnhancedGapPromotionBatchResult` 分别承接单次搜索、单 gap promotion 和 batch promotion 输出，accepted / rejected promotion 都记录 hard-gap search detail、detected gap、validation 和 selected equal model gap / promoted gap；nearby 已拆成 search context、typed candidate、typed search result、stronger test、replacement assessment、batch refinement result 和 geometry acceptance，并只把 refinement detail 与 pre-refinement gaps 交给 detection build；edge-pair / enhanced / nearby 共用 `gap_refinement_batch_detail()` 组装 accepted / rejected / searched batch detail 和 count；confidence cap / scoring 保留在 detection build / assessment 消费层；refinement helper 只消费 `gap_methods.py` 的 method family vocabulary，不 import detection evidence。 |
...
|---|---|
| `policies.formats` | Format-specific parameter overrides; they must not declare gates, detectors, diagnostics, or runtime presets. |
...
| `policies.assembly` | Build runtime policy presets from format id, physical facts, and parameter overrides. |
...
| Policy activation | format facts and parameter overrides | `formats/`, `policies/formats/format_*.py` | Physical facts and numeric search tolerances must stay separate from gate profiles, detector/runtime presets, diagnostics, and universal capability activation. |
