...
| 逻辑族群视角 | 检测逻辑本身有哪些族群，哪些已审？ | 建立 Detection / Gate / Risk 人工审核索引，Pre-detection、outer、Gap / Separator 结构和 Content 结构已完成。 | 当前接点明确为 Content 后的 candidate build / frame geometry。 | 进行中 | 进入 candidate build / frame geometry。 |
...
| Content | content evidence 是否只验证或挑战候选？ | 完成 ContentSignal / ContentRegion / ContentEvidence / ContentProposal 分层；content-only proposal detail 明确写出 weak proposal、content-model gap 和 review-only contract。 | Content 只生成 soft evidence、region hint 和 content-model candidate；不生成 hard separator、不修 outer、不 PASS / REVIEW。 | 结构完成，阈值待审 | 进入 candidate build / frame geometry；content 阈值和 conflict 逻辑后续随 scoring / gate 审核。 |
...
| Content | signal, region hints, evidence, content candidate | `image/evidence.py`, `detection/evidence/content_signal.py`, `detection/evidence/content_regions.py`, `detection/evidence/content_evidence.py`, `detection/candidate/proposal/content.py` | Content validates or challenges candidates and can create weak content-model proposals, but it cannot auto-pass alone. `content_signal.py` owns content evidence gray / float signal and threshold helpers; `content_regions.py` owns bbox/run hints; `content_evidence.py` assesses existing frame boxes; `content.py` creates review-only content candidates with content-model gaps. |
