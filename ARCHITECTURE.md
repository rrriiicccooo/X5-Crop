...
| 逻辑族群视角 | 检测逻辑本身有哪些族群，哪些已审？ | 建立 Detection / Gate / Risk 人工审核索引，Pre-detection 和 outer 已完成，Gap / Separator 已开始。 | 当前接点明确为 Gap / Separator。 | 进行中 | 从 `separator_profile` / `find_detected_gap` 继续。 |
...
| Gap / Separator | separator、geometry、content gap 证据如何分类和消费？ | 已完成族群分类，并按 candidate proposal 模型建立 `detection.candidate.proposal.separator` proposal / refinement / evidence 入口；candidate build 层已抽出 separator gap lifecycle helper；普通 separator profile / gap search、edge-pair / enhanced / nearby refinement、robust grid refinement 和 hard-gap trust 分类已拆成可审核 helper；普通 detected gap search 已把 band evidence、acceptance 和 ranking 分开，并把每个 expected gap 的 window、accepted / rejected run 和 selected hard/equal method 写入 detail；broad-width detected gap search 已把 run、width acceptance、candidate scoring 和 ranking 分开，并把每个 expected gap 的 width-profile accepted / rejected run 和 selected detected gap 写入 detail；standard 与 broad-width search 共用 `geometry/gap_search_detail.py` 生成 run evaluation summary；nearby correction 已把 searched / stronger / geometry accepted-rejected 分开；enhanced gap promotion / nearby 的关键阈值已进入 policy detail；assessment / decision / diagnostics / frame edge fitting 消费层已统一使用 `gap_methods.py` 的 gap method vocabulary 和 concrete predicate；ordinary gap search 与 broad-width gap proposal 的关键搜索参数已外显；profile / edge-refine / enhanced profile cache 已去掉 format identity；nearby correction 不再在 separator build 中调用 scoring。 | separator 只提出或修正 gap evidence；当前 hard gap 只有 `detected / edge-pair / enhanced-detected`；model gap 为 `grid / equal / content`；outer -> gaps 与 gaps -> frames 的职责已拆开；nearby 修正前后 gap lifecycle 由 candidate build 交给 detection build / assessment 消费；profile signal、gap search acceptance、gap search candidate ranking、standard gap search detail、broad-width run acceptance、shared run evaluation detail、broad-width gap search detail、edge-pair replacement、enhanced gap promotion、nearby replacement assessment、grid fit / adjustment、runtime / diagnostic hard-gap trust、gap method 消费 vocabulary、ordinary gap search 参数、broad-width search 参数和 cache key ownership 可分段审核。 | 进行中 | 继续审 separator refinement 阈值策略是否还能合并、收窄或推广。 |
...
| Outer proposal | separator-derived outer | `detection/candidate/proposal/outer/separator.py`, `detection/candidate/proposal/outer/separator_bands.py` | separator-derived outer 是否按 `outer_scope`（local / full-width）与 `gap_search_profile`（standard / broad_width）组合生成候选；broad_width 是 separator gap/search/evidence profile，不是 outer variant；标准 strip 的 full 全部 eligible，partial 只有显式 count 时 eligible extension profiles；candidate execution budget 可在可靠 primary 结果后跳过 extension 执行。 |
| Outer proposal | grid-refined outer | `detection/candidate/proposal/outer/grid_refine.py`, `detection/candidate/build/detection.py` | grid-derived outer box 计算属于 outer proposal helper；candidate build 在 primary separator gaps 与 late separator refinements 之间决定是否用 refined outer 重建 gaps；separator gap lifecycle 不拥有 outer box 修正规则。 |
...
| Gap / separator | separator cache | `geometry/separator_cache.py`, `detection/evidence/evidence_cache_keys.py`, `detection/cache_keys.py` | profile / edge-refine / enhanced profile cache key 只包含 box 与参数对象；layout 由 `AnalysisCache` 作用域承担，format identity 不进入底层 cache key；diagnostic cache key 必须包含实际影响搜索的 diagnostic policy。 |
| Gap / separator | separator proposal | `detection/candidate/proposal/separator/proposal.py`, `detection/candidate/proposal/separator/model.py`, `geometry/model_gaps.py`, `geometry/gap_search.py`, `geometry/gap_search_detail.py`, `geometry/gap_geometry.py`, `geometry/separator_band.py`, `geometry/separator_width_profile.py` | normal / broad-width gap proposal 是否只提出 gap evidence；`gap_search.py` 只负责从 profile 中找 standard detected hard gap，window、width limits、thresholds、band expansion、band evidence measurement、detected acceptance、detected candidate ranking 和 best-candidate selection 已拆成独立 helper，且关键阈值归 `GapSearchParameters`；standard search window 用 `GapSearchWindow` 承接，不再裸 tuple 传递；standard search detail 明确记录每个 expected gap 的 window、accepted / rejected run、selected detected gap 或 equal model gap，不改变 selection；单个 expected gap proposal 用 `SeparatorGapProposal` 承接 gap 与 detail，单个 detected run 使用 `DetectedGapRunAssessment` 承载 candidate 与 detail，batch candidate search 用 `DetectedGapCandidateSearchResult` 承接 candidates / evaluations，不保留 no-detail wrapper 或裸 tuple 返回；acceptance 只判断 prominence 与 width-profile support 是否足够，ranking 只比较 distance / quality / score，不混入 PASS / REVIEW 语义；`GapSearchContext` 收束单个 expected gap 的 local profile、width limits、thresholds、acceptance limits 和 ranking weights，底层 run assessment 不直接读取完整 flat config；显式 equal / grid / content model gap 生成归 `geometry/model_gaps.py`，standard gap 找不到 detected 时由 `separator/proposal.py` 调用 `separator/model.py` 生成 equal model gap，避免 hard search 和 model evidence 混在一起；detected-geometry equal model 由 `SeparatorPolicy.model_gap_proposal` 显式声明，不再借用 gate 的 geometry-support policy 字段驱动 proposal；standard 与 broad-width proposal 共享 `SeparatorGapProfileProposal` 和 profile dispatch 入口，底层搜索算法仍分别归 `geometry/gap_search.py` 与 `geometry/separator_width_profile.py`，但 accepted / rejected run summary 统一归 `geometry/gap_search_detail.py`；gap 几何约束、width CV 和局部几何误差归 `gap_geometry.py`，供 grid / nearby refinement 消费；普通 separator-derived outer bands 与 broad-width bands 共用 `SeparatorBand` 物理表示，outer band collection 用 `SeparatorOuterBandCollection` 承接；width profile 纯数学归 `geometry/separator_width_profile.py`，搜索参数归 `SeparatorPolicy.width_profile_search`，启用、max width、confidence cap、候选和 selection 策略归 `SeparatorPolicy.width_profile`，outer policy 只保留 separator-derived outer family/band 参数；proposal helper 只接收 gap search 或 width profile 参数，不接收完整 `DetectionPolicy`。 |
| Gap / separator | hard-gap trust | `geometry/gap_trust.py`, `geometry/nearby_separator.py`, `detection/evidence/gap_diagnostics.py` | hard gap 可信度是否能区分真实片间空隙、frame border、content edge；像素 signal、runtime trust assessment 和 diagnostic trust assessment 已集中在 `geometry/gap_trust.py`；runtime / diagnostic assessment 共用 `HardGapTrustContext` 与 `hard_gap_trust_assessment_result()` 组装 `HardGapTrustAssessment` detail，记录 trust label、reason、width / model delta 和 signal flags，不保留 string-only classifier wrapper；nearby candidate search / ranking / stronger detail 由 `geometry/nearby_separator.py` 提供，runtime hard-gap trust 和 correction 都直接消费 `NearbySeparatorReplacementAssessment`，不保留 accepted-only replacement wrapper；runtime correction detail 记录每个 gap 的 searched / no-candidate / not-stronger / geometry rejection / accepted，不改变 correction 条件；diagnostics 只负责采样、缓存和记录，不重复搜索或分类规则。 |
| Gap / separator | separator refinement | `geometry/edge_refine_profile.py`, `geometry/edge_pairs.py`, `geometry/nearby_separator.py`, `geometry/enhanced_separator.py`, `geometry/gap_refinement_detail.py`, `detection/candidate/build/separator_refinements.py`, `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/detection.py` | edge-pair / nearby / enhanced 只能修正或补充 gap evidence；不得直接 PASS；candidate build 直接调用 geometry refinement helpers，不保留只做别名转发的 proposal refinement facade；`separator_refinements.py` 承接 edge-pair、robust-grid、enhanced 和 nearby refinement wrappers，`separator_gaps.py` 只编排 initial source selection、primary build 和 late refinement attachment；candidate build 用 `PrimarySeparatorRefinementResult` 承接 edge-pair correction 与 robust-grid detail，用统一 `GapRefinementResult` 承接 edge-pair、enhanced 和 nearby wrapper 输出，detail 继续写入原有 report key；refinement wrapper 统一补齐 `family` 字段，pending / skipped detail 共用同一 detail helper，skip reason 区分 not-full、single-frame、policy disabled、auto not needed 和 detected-geometry model 已启用等情况；edge-pair 已明确为 gap correction，detail key 为 `edge_pair_correction`，并已拆成 edge-refine profile、profile cache owner、`EdgePairSearchLimits`、typed candidate generation、best-pair selection、`edge_pair_gap_from_candidate`、replacement assessment 和 batch refinement result；edge-pair detail 现在记录 search limits、candidate count、ranked selected candidate 和 replacement assessment，accepted / rejected replacement 都记录 reason、delta、shift / quality limit，方便人工审核是否过度替换；policy assembly 会把 format preset 转成 `EdgePairParameters`，`geometry/edge_pairs.py` 只消费 profile arrays 和基础参数对象；enhanced 已明确为 gap promotion：detected gap validation 后把 geometry model gap 提升为 `enhanced-detected`，detail key 为 `enhanced_gap_promotion`，active search 入口只保留 `find_enhanced_gap_with_detail()`，不保留无 detail / bool wrapper；`EnhancedGapSearchResult`、`EnhancedGapPromotionResult` 和 `EnhancedGapPromotionBatchResult` 分别承接单次搜索、单 gap promotion 和 batch promotion 输出，accepted / rejected promotion 都记录 hard-gap search detail、detected gap、validation 和 selected equal model gap / promoted gap；nearby 已拆成 search context、typed candidate、typed search result、stronger test、replacement assessment、batch correction result 和 geometry acceptance，并只把 correction detail 与 pre-correction gaps 交给 detection build；edge-pair / enhanced / nearby 共用 `gap_refinement_batch_detail()` 组装 accepted / rejected / searched batch detail 和 count；confidence cap / scoring 保留在 detection build / assessment 消费层；refinement helper 只消费 `gap_methods.py` 的 method family vocabulary，不 import detection evidence。 |
| Gap / separator | robust grid model gaps | `geometry/gap_geometry.py`, `geometry/robust_grid.py`, `geometry/model_gaps.py`, `detection/candidate/build/separator_gaps.py` | grid 只能补模型证据；weak grid 不得单独获得 auto PASS；candidate build 直接调用 robust grid helper；grid 已拆成 reliable anchor selection、fit candidate generation、fit ranking、fit assessment、predicted center 和 hard-gap protection / override adjustment；selected fit 和 assessment 直接消费 `GridFitCandidate`，per-gap adjustment 用 `GridGapAdjustmentResult` 承接，batch refinement 用 `RobustGridResult` 承接，不再经过裸 tuple 中转；runtime detail 明确写出 `family=robust_grid`、`evidence_kind=grid_model_gap`、`model_gap_method=grid`、rejection reason、reliable anchors、fit candidates、selected fit、fit assessment 和每个 gap 的 keep / protect / override / model action，方便审核 grid 如何覆盖弱 hard gap；`grid_model_gap` 统一生成 grid method identity；gap geometry helper 只提供几何约束，不寻找 separator；robust grid 不接收 format identity，只消费 gaps、origin/pitch、strip mode 和参数对象，并通过 `gap_methods.py` 判断 hard evidence family。 |
| Gap / separator | gap method consumers | `constants.py`, `gap_methods.py`, `detection/evidence/separator_summary.py`, `detection/candidate/assessment/*`, `detection/candidate/selection/*`, `detection/decision/pass_review.py`, `detection/evidence/*`, `geometry/frame_fit.py`, `debug/gaps.py` | `constants.py` 只声明真实 method id；`gap_methods.py` 统一 hard / geometry-model / content-model / separator-support family、具体 method predicate 和 diagnostics role vocabulary；`separator_summary.py` 只做 gap-method evidence summary。assessment、selection、decision、risk/read-only diagnostics、frame edge fitting、debug gap overlay、standard gap search 和 broad-width gap search 不重新手写 separator / model method 字符串；业务消费者不直接 import `HARD_GAP_METHODS`，统一通过 `is_hard_gap_method()` 读取 hard-family 语义；detected、edge-pair、enhanced、grid、equal、content 等具体分类通过 `gap_methods.py` predicate 消费，proposal 成功 reason 使用同一 method id 常量；raw gap 消费通过 `GapMethodEvidenceSummary` 明确区分 hard separator、grid model、equal model、content model、geometry model 和 separator support，并集中提供 hard-gap indexes、edge-pair scores、detected scores 和 leading-grid scores；width-profile selection、separator gate 与 lucky-pass risk 不再自己重数或重命名 gap method；separator gate detail 消费通过 `SeparatorGateDetailSummary` 明确区分 expected、hard separator、grid model、equal model 和 content model，避免把 grid/content model support 误读成真实 detected hard gap。 |
| Gap / separator | gap search profile planning | `detection/gap_profiles.py`, `detection/candidate/plan/separator_width_profile.py`, `detection/candidate/plan/sources.py` | active gap search profile vocabulary 只有 `standard` 与 `broad_width`；两者作为同一候选计划里的 gap search profiles；full 和显式 count partial 可进入统一 assessment，但 primary separator 已可靠时可以不执行 broad-width profile；auto-count partial 不让 broad-width profile 抢候选；profile detail helper 只消费 separator policy。 |
| Gap / separator | broad-width detected gaps | `detection/candidate/proposal/separator/proposal.py`, `detection/evidence/separator_width.py`, `detection/candidate/selection/separator_width_profile.py`, `geometry/separator_band.py`, `geometry/separator_width_profile.py` | broad separator width gaps 是否只作为普通 hard separator evidence 加 width detail，不生成独立 gap method；profile / band / expected-gap search 使用同一套底层 helper；采样上限、expected-gap 搜索窗口、run object、width acceptance、candidate scoring、rank key、best candidate selection、core-width clipped detected-gap output、距离惩罚和 score base 都在 width profile parameters / report detail 中可见；width bounds、gap search window 和 band collection 分别用 `SeparatorWidthBounds`、`SeparatorWidthGapWindow` 和 `SeparatorWidthBandCollection` 承接，且 `SeparatorWidthGapWindow` 自己暴露 empty contract；单 run assessment、batch candidate search 和 best candidate selection 分别用 `SeparatorWidthGapCandidateAssessmentResult`、`SeparatorWidthGapCandidateSearchResult`、`best_separator_width_gap_candidate` 和 `SeparatorWidthGapBestCandidateResult` 承接，不再靠裸 tuple 传递 candidate / evaluations；`separator_width_profile_gap_search` 记录每个 expected gap 的 accepted / rejected width run 和 selected detected gap，不改变 selection；未使用的无 detail wrapper 已删除，active search 只保留带 detail 的审核入口；full 模式 width-profile selection 只用 `separator_support_gaps = hard_separator_gaps + grid_model_gaps` 判断 separator 支撑，不把 content/equal model gap 当成 separator support；`separator_width_evidence` 由 candidate build 生成一次，`detection/evidence/separator_width.py` 负责 summary detail，gate / partial-safe 只改写当前 requirement 并消费 detail；outer proposal 层只看到 `SeparatorBand`，不区分 legacy wide/dark family。 |
| Gap / separator | separator gap lifecycle in candidate build | `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/separator_sources.py`, `detection/candidate/build/separator_refinements.py`, `detection/candidate/build/detection.py` | `build_detection_for_outer` 是否只消费 separator gap lifecycle 结果去生成 frames、score 和 detail；separator lifecycle 已拆成 primary gap build（origin/pitch、standard/broad-width gaps、edge-pair、grid）和 late refinements（enhanced、nearby）；initial gap source selection 已拆到 `separator_sources.py`，用 `InitialSeparatorGapResult` 承接 standard、broad-width selection 和 detected-geometry model source selection，不再裸 tuple 传递；`standard_gap_search` detail 记录 `selected_gap_source` 而不是 override 语义；`separator_gaps.py` 不再持有 source-selection 或 refinement 细节，只负责组装 `SeparatorGapBuildResult`；primary refinement 用 `PrimarySeparatorRefinementResult` 承接 edge-pair 与 grid 输出；late refinement 用 `LateSeparatorRefinementResult` 承接 enhanced promotion、nearby correction、最终 gaps 和 pre-nearby gaps；late refinement 只消费 `analysis_mode`，不持有完整 `RuntimeConfig`；grid-derived outer refine 由 candidate build 在两段之间重新 build，不由 separator lifecycle 决定 PASS / REVIEW 或 outer 修正规则。 |
...
| Content | content evidence | `detection/evidence/content_evidence.py` | `ok / weak / low_content / aspect_conflict` 是否符合照片内容和 aspect；cached / uncached 路径共用 threshold 与 frame support summary helper，坐标转换和缓存归各自入口处理。 |
...
| Content | content candidate | `detection/candidate/proposal/content.py`, `geometry/model_gaps.py` | content candidate 只能生成 content-model gap 和候选线索；content-only candidate 必须 review-only，除非未来显式改变 final contract。 |
...
| Candidate scoring | separator support score | `detection/candidate/assessment/scoring.py`, `detection/evidence/separator_summary.py` | hard/grid/equal 的信用是否符合 hard > model 的原则；raw gap scoring 和 width-profile selection 使用 `GapMethodEvidenceSummary`，gate-detail scoring 使用 `SeparatorGateDetailSummary`；`detected_gaps` 是兼容 report 字段，内部使用 `separator_support_gaps = hard_separator_gaps + grid_model_gaps`，不得把 grid/content model gap 命名为 hard detected evidence。 |
...
| Gate | leading grid failure | `detection/candidate/assessment/gates.py` | 前段 grid 弱、hard gap 偏后时是否阻止 lucky pass；leading grid scores、late adjacent hard-gap sequence、enhanced promotion guard 已拆成独立 helper，最终 reason 保持 `leading_grid_separator_failure`。 |
...
| Gap / separator | profile, cache, proposal, hard trust, refinement, grid model | `gap_methods.py`, `geometry/separator_*`, `geometry/model_gaps.py`, `geometry/edge_refine_profile.py`, `geometry/gap_search.py`, `geometry/gap_search_detail.py`, `geometry/gap_geometry.py`, `geometry/gap_trust.py`, `geometry/nearby_separator.py`, `detection/evidence/separator_width.py`, `detection/candidate/proposal/separator/*`, `detection/candidate/build/separator_sources.py`, `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/separator_refinements.py` | Hard evidence must stay stronger than model/equal/grid/content evidence, and cache keys must include policy-relevant context. Separator proposal creates gap evidence; geometry refinement helpers adjust or promote it; neither can PASS / REVIEW. Base separator profile generation, explicit `SeparatorProfileSignals`, combined profile scoring, explicit equal / grid / content model-gap generation, edge-pair edge-refine profile generation, profile cache ownership, hard-only gap-search band evidence measurement, shared run evaluation summary detail, shared `SeparatorGapProposal` contract for single expected-gap proposals, shared `SeparatorGapProfileProposal` contract for standard and broad-width profile proposals, standard detected candidate search result contract, shared `PrimarySeparatorRefinementResult` contract for edge-pair / robust-grid outputs, shared `GapRefinementResult` wrapper contract and shared family/pending/skipped detail assembly for edge-pair / enhanced / nearby refinements, edge-pair search / replacement / refinement result contracts, broad-width bounds / window / band collection contracts, broad-width run / candidate-search / best-candidate result contracts, width evidence requirement detail, typed separator bands, gap geometry metrics, edge-pair replacement assessment with accepted/rejected reasons, enhanced detail-only search / single promotion / batch promotion result contracts, nearby search / replacement assessment / correction result contracts, grid fit / adjustment / refinement result contracts, shared runtime / diagnostic hard-gap trust context and assessment-result detail contracts, and centralized gap method family / role vocabulary are split into small helpers for review. Candidate build directly calls the geometry refinement helpers, owns cached edge/background profile retrieval, attaches separator width evidence summary from `detection/evidence/separator_width.py`, and no longer keeps a proposal refinement facade that only re-exports functions. Initial source-selection wrappers now live in `separator_sources.py`, refinement wrappers live in `separator_refinements.py`, while `separator_gaps.py` owns lifecycle result assembly. Edge-pair preset policy is assembled into `EdgePairParameters` before geometry sees it. Enhanced min score and nearby width-CV slack are explicit policy/report fields. Width profile math lives in `geometry/separator_width_profile.py`; search parameters live under `SeparatorPolicy.width_profile_search`, while activation, max width, confidence cap, candidate, and selection policy live under `SeparatorPolicy.width_profile`; outer policy only owns separator-derived outer family/band parameters. |
| Gap / separator | gap search profile planning and broad-width detected gaps | `detection/gap_profiles.py`, `detection/candidate/plan/separator_width_profile.py`, `detection/candidate/plan/sources.py`, `detection/candidate/proposal/separator/proposal.py`, `detection/evidence/separator_width.py`, `geometry/separator_width_profile.py` | Profile evidence must be explicit, capped when needed, and assessed through the normal candidate gate; reliable primary separator candidates may skip broad-width profile execution, and auto-count partial does not let broad-width profiles compete. |
| Gap / separator | separator gap lifecycle in candidate build | `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/separator_sources.py`, `detection/candidate/build/separator_refinements.py`, `detection/candidate/build/detection.py` | `build_detection_for_outer` consumes separator gap lifecycle output for frame building, scoring, and detail writing. The lifecycle is split into primary gap build (origin/pitch, standard/broad-width gaps, edge-pair, grid) and late refinements (enhanced, nearby); initial gap source selection lives in `separator_sources.py` and uses `InitialSeparatorGapResult` for standard, broad-width selection, and detected-geometry model source selection instead of raw tuple passing; `standard_gap_search` detail records `selected_gap_source` rather than override semantics; `separator_gaps.py` owns lifecycle result assembly, while `separator_refinements.py` owns edge-pair, robust-grid, enhanced, and nearby wrappers; primary refinement uses `PrimarySeparatorRefinementResult` for edge-pair and grid outputs; late refinement uses `LateSeparatorRefinementResult` for enhanced promotion, nearby correction, final gaps, and pre-nearby gaps; late refinement consumes only `analysis_mode`, not the full `RuntimeConfig`; candidate build handles any refined-outer rebuild between those stages. |
| Content | content evidence, profile runs, mask outer, content candidate | `detection/evidence/content_*`, `detection/candidate/proposal/content.py` | Content can validate or challenge candidates but must not auto-pass alone. Cached and uncached content evidence paths share threshold and frame-support summary helpers; each entry point owns only coordinate conversion and cache plumbing. |
