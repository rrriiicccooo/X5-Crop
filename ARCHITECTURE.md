...
| Gap / Separator | separator、geometry、content gap 证据如何分类和消费？ | 已完成族群分类，并按 candidate proposal 模型建立 `detection.candidate.proposal.separator` proposal / refinement / evidence 入口；candidate build 层已抽出 separator gap lifecycle helper；普通 separator profile / gap search、edge-pair refinement 和 robust grid refinement 已拆成可审核 helper。 | separator 只提出或修正 gap evidence；当前 hard gap 只有 `detected / edge-pair / enhanced-detected`；outer -> gaps 与 gaps -> frames 的职责已拆开；profile signal、gap search candidate ranking、edge-pair replacement 和 grid fit / adjustment 可分段审核。 | 进行中 | 继续审 hard-gap trust、enhanced 和 nearby refinement 的判定规则。 |
