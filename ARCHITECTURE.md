...
| Gap / separator | profile, cache, proposal, hard trust, refinement, grid model | `geometry/separator_*`, `geometry/edge_refine_profile.py`, `geometry/gap_search.py`, `geometry/gap_geometry.py`, `geometry/gap_trust.py`, `detection/candidate/proposal/separator/*` | Hard evidence must stay stronger than model/equal/grid evidence, and cache keys must include policy-relevant context. Separator proposal/refinement only creates or adjusts gap evidence; it does not PASS / REVIEW. Base separator profile generation, edge-pair edge-refine profile generation, gap-search candidate ranking, typed separator bands, gap geometry metrics, edge-pair replacement, enhanced promotion, nearby replacement, grid fit / adjustment, and runtime / diagnostic hard-gap trust classifiers are split into small helpers for review. Enhanced min score and nearby width-CV slack are explicit policy/report fields. Width profile math lives in `geometry/separator_width_profile.py`; width profile parameters live under `SeparatorPolicy.width_profile`; outer policy only owns separator-derived outer family/band parameters. |
