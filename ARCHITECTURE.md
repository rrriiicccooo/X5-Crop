...
| Gap / Separator | separator、geometry、content gap 证据如何分类和消费？ | 已完成族群分类，并按 candidate proposal 模型建立 `detection.candidate.proposal.separator` proposal / refinement / evidence 入口；candidate build 层已抽出 separator gap lifecycle helper；普通 separator profile / gap search、edge-pair / enhanced / nearby refinement、robust grid refinement 和 hard-gap trust 分类已拆成可审核 helper；enhanced / nearby 的关键阈值已进入 policy detail。 | separator 只提出或修正 gap evidence；当前 hard gap 只有 `detected / edge-pair / enhanced-detected`；outer -> gaps 与 gaps -> frames 的职责已拆开；profile signal、gap search candidate ranking、edge-pair replacement、enhanced promotion、nearby replacement、grid fit / adjustment 和 runtime / diagnostic hard-gap trust 可分段审核。 | 进行中 | 继续审 separator refinement 阈值策略是否还能合并、收窄或推广。 |
...
| Gap / separator | separator refinement | `detection/candidate/proposal/separator/refinement.py`, `geometry/edge_pairs.py`, `geometry/nearby_separator.py`, `geometry/enhanced_separator.py` | edge-pair / nearby / enhanced 只能修正或补充 gap evidence；不得直接 PASS；edge-pair 已拆成 search limits、candidate generation、best-pair selection 和 replacement eligibility；enhanced 已拆成 detected gap validation / promotion / merge；nearby 已拆成 search context、candidate ranking、stronger test 和 geometry acceptance；`enhanced.min_score` 与 `nearby_correction.width_cv_slack` 在 policy/report detail 中显式可见。 |
