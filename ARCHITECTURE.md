...
| Candidate plan | gap search profiles | `detection/gap_profiles.py`, `candidate/separator_width_profile_plan.py`, `candidate/sources.py`, `candidate/reliability.py` | `standard` 和 `broad_width` 是否作为同一候选计划的 eligible profiles；execution budget 可在 reliable primary 后跳过更贵的 profile 执行，不得退回失败补救式 retry。 |
