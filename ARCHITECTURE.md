...
- content support score 表示 containment support；content quality score 只表示影像证据强弱，
  ...
  support score 必须优先消费这些字段，不能被旧 `support: ok` fallback 覆盖。
...
- Content support score means containment support. Content quality score measures
  ...
  `content_containment_ok` / `content_harm_risk`, support score must consume
  those fields first and must not be overridden by the legacy `support: ok`
  fallback. Partial-holder checks and evidence-dependency validation
