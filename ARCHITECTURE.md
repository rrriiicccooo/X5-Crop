...
| DecisionGate | 唯一创建 `DecisionResult`、最终 status 和 `final_review_reasons`。 |
| Finalization | 只应用 `FrameBleedPlan` 和 bounds clamp，并创建 `FinalDetection`；不重新检测。 |
| Report / Debug | 只读 typed final results；不补算事实、不参与裁决。 |

`SelectionResult`、`DecisionResult` 与 `FinalDetection` 是三个独立生命周期结果：selection 拥有候选、
evidence 和 geometry resolution；decision 拥有 DecisionGate 与 decision geometry；final detection 只把
该 decision 与已应用 bleed 的 output geometry 组合起来。Report 与 Debug 显式接收所需 typed result，
不存在 cache-only optional 字段。
...
不保存 family、role、notes 或 frame-size label 等 report-only 描述。
...
Root measurement arrays 必须使用同一 validated workspace layout 与 shape。
...
| `decision` | 唯一 DecisionGate 和 `DecisionResult` factory。 |
