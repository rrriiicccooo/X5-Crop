...
| Candidate scoring | separator support score | `detection/candidate/assessment/scoring.py`, `detection/evidence/separator_summary.py` | hard/grid/equal 的信用是否符合 hard > model 的原则；raw gap scoring 使用 `GapMethodEvidenceSummary`，gate-detail scoring 使用 `SeparatorGateDetailSummary`；`detected_gaps` 是兼容 report 字段，内部使用 `separator_support_gaps = hard_separator_gaps + grid_model_gaps`，不得把 grid/content model gap 命名为 hard detected evidence。 |
