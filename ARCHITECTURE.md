...
| Gap / separator | separator proposal | `detection/candidate/proposal/separator/proposal.py`, `geometry/gap_search.py`, `geometry/separator_width_profile.py` | normal / broad-width gap proposal 是否只提出 gap evidence；gap search 的 window、width limits、thresholds、band expansion、prominence gate 和 candidate ranking 已拆成独立 helper，且关键阈值归 `GapSearchParameters`；width profile 纯数学归 `geometry/separator_width_profile.py`，参数归 `SeparatorPolicy.width_profile`，outer policy 只保留 separator-derived outer family/band 参数；proposal helper 只接收 gap search 或 width profile 参数，不接收完整 `DetectionPolicy`。 |
...
| Gap / separator | robust grid model gaps | `detection/candidate/proposal/separator/refinement.py`, `geometry/robust_grid.py` | grid 只能补模型证据；weak grid 不得单独获得 auto PASS；grid 已拆成 reliable anchor selection、grid fit、predicted center 和 hard-gap protection / override adjustment；robust grid 不接收 format identity，只消费 gaps、origin/pitch、strip mode 和参数对象，方便单独审核 model gap 何时能覆盖弱 hard gap。 |
...
| Gap / separator | profile, cache, proposal, hard trust, refinement, grid model | `geometry/separator_*`, `geometry/edge_refine_profile.py`, `geometry/gap_search.py`, `geometry/gap_trust.py`, `detection/candidate/proposal/separator/*` | Hard evidence must stay stronger than model/equal/grid evidence, and cache keys must include policy-relevant context. Separator proposal/refinement only creates or adjusts gap evidence; it does not PASS / REVIEW. Base separator profile generation, edge-pair edge-refine profile generation, gap-search candidate ranking, edge-pair replacement, enhanced promotion, nearby replacement, grid fit / adjustment, and runtime / diagnostic hard-gap trust classifiers are split into small helpers for review. Enhanced min score and nearby width-CV slack are explicit policy/report fields. Width profile math lives in `geometry/separator_width_profile.py`; width profile parameters live under `SeparatorPolicy.width_profile`; outer policy only owns separator-derived outer family/band parameters. |
