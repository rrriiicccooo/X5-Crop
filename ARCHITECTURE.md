...
candidate state. The solver consumes all seven modules. No lower owner imports the solver.
...
candidate construction 与 blank/occlusion/completion 仍由 solver 承载，等待后续独立 owner。
...
candidate state，solver 统一消费全部低层 owner；低层模块不得反向导入 solver，solver 也不兼容
导出已经迁移的符号。
