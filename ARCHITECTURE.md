...
| Gap / separator | separator proposal | `detection/candidate/proposal/separator/proposal.py`, `detection/candidate/proposal/separator/model.py`, `geometry/model_gaps.py`, `geometry/gap_search.py`, `geometry/gap_geometry.py`, `geometry/separator_band.py`, `geometry/separator_width_profile.py` | normal / broad-width gap proposal 是否只提出 gap evidence；`gap_search.py` 只负责从 profile 中找 standard detected/equal gaps，window、width limits、thresholds、band expansion、prominence gate、detected candidate collection、best-candidate selection 和 equal fallback 已拆成独立 helper，且关键阈值归 `GapSearchParameters`；显式 equal / grid / content model gap 生成归 `geometry/model_gaps.py`，profile 等分模型归 `separator/model.py`，避免在 build / safety / refinement / content candidate 路径手写 method 字符串；gap 几何约束、width CV 和局部几何误差归 `gap_geometry.py`，供 grid / nearby refinement 消费；普通 separator-derived outer bands 与 broad-width bands 共用 `SeparatorBand` 物理表示；width profile 纯数学归 `geometry/separator_width_profile.py`，参数归 `SeparatorPolicy.width_profile`，outer policy 只保留 separator-derived outer family/band 参数；proposal helper 只接收 gap search 或 width profile 参数，不接收完整 `DetectionPolicy`。 |
