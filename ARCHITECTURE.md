...
| Gap / separator | separator proposal | `detection/candidate/proposal/separator/proposal.py`, `geometry/gap_search.py`, `geometry/gap_geometry.py`, `geometry/separator_width_profile.py` | normal / broad-width gap proposal 是否只提出 gap evidence；`gap_search.py` 只负责从 profile 中找 standard detected/equal gaps，window、width limits、thresholds、band expansion、prominence gate 和 candidate ranking 已拆成独立 helper，且关键阈值归 `GapSearchParameters`；gap 几何约束、width CV 和局部几何误差归 `gap_geometry.py`，供 grid / nearby refinement 消费；width profile 纯数学归 `geometry/separator_width_profile.py`，参数归 `SeparatorPolicy.width_profile`，outer policy 只保留 separator-derived outer family/band 参数；proposal helper 只接收 gap search 或 width profile 参数，不接收完整 `DetectionPolicy`。 |
