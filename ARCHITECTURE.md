...
| `detection.separator` | separator/gap 逻辑族入口：gap proposal、separator correction、width evidence 和 separator-derived outer band evidence。 |
...
| Gap / Separator | separator、geometry、content gap 证据如何分类和消费？ | 已完成族群分类，并按 outer 模型建立 `detection.separator` proposal / correction / evidence 入口。 | separator 只提出或修正 gap evidence；当前 hard gap 只有 `detected / edge-pair / enhanced-detected`。 | 进行中 | 从 `separator_profile` / `find_gap` 开始逐项深审。 |
...
| Outer proposal | separator-derived outer | `detection/candidate/proposal/outer/separator.py`, `detection/separator/bands.py` | separator-derived outer 是否按 `outer_scope`（local / full-width）与 `gap_search_profile`（standard / broad_width）组合生成候选；broad_width 是 separator gap/search/evidence profile，不是 outer variant；标准 strip 的 full 全部 eligible，partial 只有显式 count 时 eligible extension profiles；candidate execution budget 可在可靠 primary 结果后跳过 extension 执行。 |
...
| Gap / separator | separator proposal | `detection/separator/proposal.py`, `geometry/gap_search.py` | normal / broad-width gap proposal 是否只提出 gap evidence；gap width、guard、peak、geometry constraint 是否符合当前 format/mode。 |
| Gap / separator | hard-gap trust | `geometry/gap_trust.py` | hard gap 可信度是否能区分真实片间空隙、frame border、content edge。 |
| Gap / separator | separator correction | `detection/separator/correction.py`, `geometry/edge_pairs.py`, `geometry/nearby_separator.py`, `geometry/enhanced_separator.py` | edge-pair / nearby / enhanced 只能修正或补充 gap evidence；不得直接 PASS；替换或确认 hard gap 时 score / shift 约束是否足够严格。 |
| Gap / separator | robust grid model gaps | `detection/separator/correction.py`, `geometry/robust_grid.py` | grid 只能补模型证据；weak grid 不得单独获得 auto PASS。 |
...
| Gap / separator | broad-width detected gaps | `detection/separator/proposal.py`, `detection/separator/evidence.py` | broad separator width gaps 是否只作为普通 hard separator evidence 加 width detail，不生成独立 gap method。 |
...
| Candidate | build detection for outer | `detection/candidate/build/detection.py` | outer -> gaps -> frame boxes -> confidence 的中间 detail 是否完整可审。 |
...
| Gap / separator | profile, cache, proposal, hard trust, correction, grid model | `geometry/separator_*`, `geometry/gap_search.py`, `geometry/gap_trust.py`, `detection/separator/*` | Hard evidence must stay stronger than model/equal/grid evidence, and cache keys must include policy-relevant context. Separator proposal/correction only creates or adjusts gap evidence; it does not PASS / REVIEW. |
| Gap / separator | gap search profile planning and broad-width detected gaps | `detection/gap_profiles.py`, `detection/candidate/plan/separator_width_profile.py`, `detection/candidate/plan/sources.py`, `detection/separator/proposal.py`, `detection/separator/evidence.py` | Profile evidence must be explicit, capped when needed, and assessed through the normal candidate gate; reliable primary separator candidates may skip broad-width profile execution, and auto-count partial does not let broad-width profiles compete. |
