...
| `frame_sequence_solver.py` | Remaining candidate construction, boundary-role, blank/occlusion/completion, and top-level orchestration responsibilities pending their own canonical owners. |
...
and candidate state. The solver consumes all six modules. No lower owner imports the
solver.
...
`frame_sequence_separator_assignment.py` 拥有。candidate construction、边界角色与
blank/occlusion/completion 仍由 solver 承载，等待后续独立 owner。依赖保持单向：measurement
facts 供 common-width、search、candidate state 与 separator assignment 消费；consensus 与
separator assignment 再消费 candidate state，solver 统一消费全部低层 owner；低层模块不得反向
导入 solver，solver 也不兼容导出已经迁移的符号。
