...
- candidate 子层读取或更新候选级原因必须经过 `detection.candidate.reasons`；底层
  `Detection.review_reasons` 只是 decision 覆盖前的临时兼容存储，不允许在 candidate
  子层直接 append 成 final-looking reason。
...
Candidate sublayers must read or update candidate-level reasons through
`detection.candidate.reasons`; the underlying `Detection.review_reasons` field
is temporary compatibility storage before decision overwrites it, not a place for
candidate sublayers to append final-looking reasons directly.
