...
| Gap / separator | separator gap lifecycle in candidate build | `detection/candidate/build/separator_gaps.py`, `detection/candidate/build/detection.py` | `build_detection_for_outer` 是否只消费 separator gap lifecycle 结果去生成 frames、score 和 detail；separator lifecycle 已拆成 primary gap build（origin/pitch、standard/broad-width gaps、edge-pair、grid）和 late refinements（enhanced、nearby）；grid-derived outer refine 由 candidate build 在两段之间重新 build，不由 separator lifecycle 决定 PASS / REVIEW 或 outer 修正规则。 |
