...
- candidate reason bucketing 输出改为显式 `CandidateReasonBuckets`，字段化 `blockers`
  和 `diagnostics`，候选 reason taxonomy 不再用匿名二元组传递。
- decision low-confidence context 输出改为显式 `DecisionContextReviewAssessment`，
  字段化 `reasons` 和 `reason_inputs`，最终 context reason 与输入证据不再用匿名 tuple
  携带。
...
- Candidate reason bucketing now returns an explicit `CandidateReasonBuckets`
  with `blockers` and `diagnostics` fields, so candidate reason taxonomy no
  longer travels as an anonymous pair.
- Decision low-confidence context now returns an explicit
  `DecisionContextReviewAssessment` with `reasons` and `reason_inputs` fields,
  so final context reasons and their input evidence no longer travel as an
  anonymous tuple.
