...
当前 active 脚本：`X5_Crop.py` V3.6.2
...
| 版本 | 状态 | 摘要 |
|---|---|---|
| V3.6.2 | 当前 active 开发版 | 合并低收益 `equal-broad-region` method，并把 hard fallback 缩小成 review-only equal split fallback。 |
...
Current active script: `X5_Crop.py` V3.6.2
...
|---|---|---|
| V3.6.2 | Current active development | Folds the low-value `equal-broad-region` method into ordinary `equal`, and shrinks hard fallback into a review-only equal split fallback. |
