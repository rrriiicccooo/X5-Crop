...
- Frame content 聚合已从误导性的 containment/integrity 字段改为
  ...
  可用性，不能伪装成真实内容受损。
- 当前 report schema revision 为 `physical_gate_proof_paths`；report/debug/cache reuse 不保留
...
- The current report schema revision is `physical_gate_proof_paths`; report, debug,
