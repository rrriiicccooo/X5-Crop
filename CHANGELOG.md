...
- 入口和运行层收敛为 `entry`、`runtime.config`、`runtime.input_probe`、`runtime.app` 和
  `runtime.workflow`。
- format physical facts 由 `x5crop.formats` 承担；runtime profile 名由 physical traits 推导。
...
- 新增 `ScanCalibration` / `PhysicalLength` / `PixelKernel` 单位模型；TIFF
...
- runtime policy、policy assembly 和 final decision contract 分层；runtime 边界创建
  `DetectionPolicyBundle` 并一次性解析当前 policy、format 和 dual-lane 支撑项；detection
  只读取 bundle，不再动态回查 policy / format registry。
- dual-lane 的 lane count 和 lane format 已从 detector policy 默认值迁入
  ...
  diagnostics 和 report surface 也必须由 assembly 显式装配。
...
- candidate lifecycle 进一步拆清：`candidate.plan` 只声明 count、offset、source descriptors
  ...
  Detection geometry，geometry evidence enrichment 显式发生在 assessment 前。
...
- `FormatSpec` 进一步收敛为纯物理规格；runtime profile 名称不再存放在 format spec 上，
  由 `policies.assembly` / `policies.reporting` 从 physical facts 推导。
...
- report schema revision 更新为 `physical_decision_output_bleed`；cached analysis reuse 只接受
  当前 report record、current decision summary 和当前 output geometry detail。
- decision layer 生成 canonical final status、final confidence 和
  `final_review_reasons`；decision contract 由上层显式传入，decision 不再从完整 runtime
  policy 自行组装 contract。
...
- report schema identity 从版本号式命名收敛为 `schema_id` + `schema_revision`，
  避免脚本版本、policy 和 report schema 混在同一个名称里。
- output overlap 从 final review blocker 收敛为 output-protection evidence：detected、
  required output bleed、available output bleed、bleed sufficiency、protected 和 unresolved
  分开记录；可由 bleed 保护的 overlap 按 required bleed 扩大输出长轴范围；只有
  `output_overlap_unresolved` 才能进入 DecisionGate。
...
- regression compare 工具默认作为 diff 审计工具返回 0；只有显式 `--fail-on-diff` 才把
  reference diff 作为命令失败。
...
- Format physical facts belong to `x5crop.formats`; runtime profile names are
  derived from physical traits. The old per-format parameter modules and string
...
- `ScanCalibration` / `PhysicalLength` / `PixelKernel` now define unit ownership;
...
- Runtime policy, policy assembly, and final decision contract are separated.
  Runtime creates a `DetectionPolicyBundle` at the boundary, and detection no
  longer imports a runtime policy context.
...
- Candidate lifecycle is stricter: `candidate.plan` only declares count, offset,
  ...
  `candidate.execution`, build only creates unscored Detection geometry, and
...
- `FormatSpec` is now a pure physical spec; runtime profile names are derived
  from physical facts in `policies.assembly` / `policies.reporting` instead of
  being stored on the format spec.
- The decision layer produces canonical final status, final confidence, and
  `final_review_reasons`; the decision contract is passed in explicitly and is
  no longer assembled by the decision layer from the full runtime policy.
...
- Report schema revision is now `physical_decision_output_bleed`; cached
  analysis reuse requires the current report record, current decision summary,
  and current output geometry detail.
- Report schema identity now uses `schema_id` + `schema_revision` instead of a
  version-styled name, keeping script version, policy, and report schema separate.
- Output overlap is now output-protection evidence instead of a final review
  blocker: detected, required output bleed, available output bleed, bleed
  sufficiency, protected, and unresolved are recorded separately; bleed-coverable
  overlap expands long-axis output by the required bleed, and only
  `output_overlap_unresolved` can enter DecisionGate.
...
- Regression compare tools now default to diff-audit behavior with exit 0;
  `--fail-on-diff` is required to turn a reference diff into command failure.
