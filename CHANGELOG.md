...
- format parameter ownership 已进一步收紧：`policies/parameters/ownership.py`
  将 format override 拆成 `FormatToleranceProfile`、
  `FormatContentProfileTolerance` 和 `SearchBudgetPolicy`，并限制
  format 文件只能覆盖 physical tolerance、content profile tolerance 和 search
  budget 字段；separator pixel signal threshold 不得按 format 覆盖；
  count inclusion、gate supplemental
...
- separator evidence 消费已集中到 `detection/evidence/separator_summary.py`：
  ...
  `separator_width`、`outer_alignment` 和 outer geometry correction 也不再直接
  import `HARD_GAP_METHODS`，统一通过 `is_hard_gap_method()` 消费 hard-family
  语义；
...
- Separator evidence consumption is centralized in
  ...
  separator gate no longer rescans gap methods itself. `separator_width`,
  `outer_alignment`, and outer geometry correction also consume hard-family
  semantics through `is_hard_gap_method()` instead of importing
  `HARD_GAP_METHODS` directly, while
...
- Runtime separator width-profile policy is consolidated: activation, max width
  ...
  fields.
- Separator gap-profile proposal contract is unified: standard and broad-width
