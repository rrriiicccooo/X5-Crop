...
当前 active 脚本：`X5_Crop.py` V3.6.3
...
| 版本 | 状态 | 摘要 |
|---|---|---|
| V3.6.3 | 当前 active 开发版 | 将叠片 / 近似叠片作为困难图处理：135 full strip 的 strong overlap-risk model gap 会进入 REVIEW。 |
| V3.6.2 | 开发版 | 合并低收益 `equal-broad-region` method，并把 hard fallback 缩小成 review-only equal split fallback。 |
...
Current active script: `X5_Crop.py` V3.6.3
...
|---|---|---|
| V3.6.3 | Current active development | Treats overlap / near-overlap as difficult: strong overlap-risk model gaps in 135 full strips are sent to REVIEW. |
| V3.6.2 | Development | Folds the low-value `equal-broad-region` method into ordinary `equal`, and shrinks hard fallback into a review-only equal split fallback. |
