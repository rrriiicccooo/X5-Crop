...
- `status`、`confidence`、`final_review_reasons`、`outer_box`、`frame_boxes`、`gaps`、
  `detail.policy` 和 `report_schema` 都可以出现 diff；需要时记录原因和涉及层级。
...
- decision evidence policy 已从 format-id override 表收敛为 physical trait 推导；format 名字只作为
  `FormatSpec` 查询入口。
- image / geometry foundation helper 改为显式参数对象；image evidence 的默认值只通过命名
  foundation factory 暴露。
...
- `status`, `confidence`, `final_review_reasons`, `outer_box`, `frame_boxes`,
  `gaps`, `detail.policy`, and `report_schema` may all differ; when useful,
...
- Image / geometry foundation helpers now use explicit parameter objects, with
  defaults exposed only through named foundation factories.
