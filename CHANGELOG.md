...
- runtime decision policy 已从 finalization policy 中拆出；confidence cap、tail review
...
- content mismatch selector 的 policy、字段和 helper 名称收敛为 candidate-selection 语义；
  它只读取 candidate-level reasons 选择候选，不生成最终 review reason。
...
- safety candidate 的 review-only contract、candidate cap 和 auto-gate 改写从
...
- decision summary 不再输出旧 `review_reasons_added` 字段；最终 reason 增量写入
  `final_review_reasons_added`，包括最后补充的 decision tail reason。
...
- `python3 -m unittest discover -s tools/tests` 通过 68 个 unit / contract tests。
- `git diff --check` 通过。
...
- Runtime decision policy has been split out of finalization policy. Confidence
  caps, tail review reasons, and the outer-content alignment evidence switch now
  belong to the decision surface.
...
- Safety-candidate review-only contract, candidate cap, and auto-gate rewrite
...
- `python3 -m unittest discover -s tools/tests` passed 68 unit / contract tests.
