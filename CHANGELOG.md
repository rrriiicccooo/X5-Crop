...
- `FormatPhysicalSpec` 成为唯一物理规格类型；旧 `FormatSpec` 别名和 format-name aspect map
  已删除，runtime profile 名称不再存放在 format spec 上，
...
- report schema revision 更新为 `physical_count_and_output_protection`；cached analysis reuse
  只接受含 current count selection、exposure evidence、output protection plan、decision summary
  和 output geometry 的当前 report record。
...
- 叠片链路拆成纯 `exposure_overlap_evidence` 与 `OutputProtectionPlan`：evidence 只测量
  ...
  该物理能力对所有 format/mode 通用，不再由 format trait 开关；即使 plan 不完全可行，
...
- `FormatPhysicalSpec` is the sole physical-spec type. The old `FormatSpec` alias
  and format-name aspect map are gone; runtime profile names are derived from
  physical facts in `policies.assembly` / `policies.reporting`.
...
- Report schema revision is now `physical_count_and_output_protection`; cached
  analysis reuse requires current count selection, exposure evidence, output
  protection plan, decision summary, and output geometry.
...
- Exposure overlap is split into pure `exposure_overlap_evidence` and an
  ...
  format/mode combinations rather than trait-switched; an infeasible REVIEW output
