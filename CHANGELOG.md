...
当前 active 脚本：`X5_Crop.py` V3.3.1
...
| V3.3.1 | 当前 active / 稳定发布版 | 稳定打包版本，基于 V3/V3.2 风格检测链路，并加入只在输出阶段生效的 bleed。 |
...
V3.3.1 现在重新作为 active 版本，因为它是已打包发布、已知更稳的基线。V3.3.2、V3.4.1、V3.4.2 和 V3.5 都提供过有价值的方向，但它们会改变一些已知准确样本的最终输出，因此当前先回到 V3.3.1。
...
- 它不包含后续的 V3.4.1 hard/grid conflict 处理、V3.4.2 local-grid 实验或 V3.5 hard-gap semantic validation 实验。
- 叠片、片距不稳定、红框/grid 冲突和内部边缘误判仍然是后续优化方向。
- 之后的修复应该更窄，并且在改变输出前必须先通过已知准确样本保护。
...
### V3.3.1：当前 Active / 稳定发布版

状态：当前 active 脚本 / GitHub 稳定发布版。
...
Current active script: `X5_Crop.py` V3.3.1
...
| V3.3.1 | Current active / Stable Release | Stable packaged release based on V3/V3.2 style detection plus output-only bleed. |
...
### Current Active: V3.3.1

V3.3.1 is active again because it is the packaged, better-known stable baseline. V3.3.2, V3.4.1, V3.4.2, and V3.5 all provided useful directions, but they also changed the final output on some known-good scans, so the active script is back to V3.3.1 for now.
...
- It is the best current regression baseline.
- It better protects final output on known-good scans.
- Future improvements can start as diagnostics or review-only layers before they are allowed to alter output boxes.

Known limitations:

- It does not contain the later V3.4.1 hard/grid conflict handling, V3.4.2 local-grid experiment, or V3.5 hard-gap semantic validation experiment.
- Overlap, irregular spacing, red/grid conflict, and internal-edge false positives remain future optimization targets.
- Future fixes should be narrower and must protect known-good scans before changing output.
...
### V3.3.1: Current Active / Stable Release

Status: current active script / stable GitHub Release.
