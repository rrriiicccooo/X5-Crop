...
当前 active 脚本：`X5_Crop.py` V3.6.1
...
| 版本 | 状态 | 摘要 |
|---|---|---|
| V3.6.1 | 当前 active 开发版 | 继续优化诊断层：诊断报告和 Debug Analysis 诊断 tick 只在显式 `--diagnostics` 时生成；普通启动器不开启诊断。 |
...
- 本地测试可使用未同步的诊断启动器；它默认 `deskew off`、`dry run`、`debug analysis` 和 `--diagnostics`。
...
Current active script: `X5_Crop.py` V3.6.1
...
|---|---|---|
| V3.6.1 | Current active development | Continues the diagnostic layer: diagnostic report data and Debug Analysis diagnostic ticks are only generated with explicit `--diagnostics`; normal launchers do not enable diagnostics. |
...
- A local-only diagnostic launcher can be used for testing; it defaults to `deskew off`, `dry run`, `debug analysis`, and `--diagnostics`.
