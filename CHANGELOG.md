...
- `ARCHITECTURE.md` 现在放在仓库根目录，并与 `docs/ARCHITECTURE.md` 保持同内容镜像；文档改为中文说明 + English Guide，方便从 GitHub 根目录直接阅读开发架构地图。
...
- Adds root-level `ARCHITECTURE.md` as a bilingual developer architecture guide,
  mirrored from `docs/ARCHITECTURE.md`, so the source layout, policy ownership,
  format/mode isolation, and regression boundaries are readable from the GitHub
  root.
