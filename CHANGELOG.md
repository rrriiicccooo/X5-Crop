...
- Runtime policy, policy assembly, and final decision contract are separated.
  Runtime fully resolves the active format/policy and dual-lane support entries
  into a `DetectionPolicyBundle`; detection only reads that bundle and cannot
  query policy or format registries.
