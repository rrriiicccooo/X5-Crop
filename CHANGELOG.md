...
- 本轮不保留旧字段、旧别名、旧 reducer、旧 schema 或动态 import 约定；输出 diff 不作为
  阻塞条件。
- `final_review_reasons` 成为唯一 canonical final reason 字段；JSONL、CSV、report、debug
  和 cache reuse 都读取当前 schema。
- `candidate_assessment.candidate_gate.passed` 成为候选 gate 唯一 canonical 结果；
  `candidate_gate_passed` 投影字段已删除。
- partial safety 语义收敛为 `partial_edge_safety` / `partial_holder_safety`；空 frame 是允许
  输出结果，不再作为 gate 名称。
- `DecisionAssessmentInput -> DecisionGateAssessment` 是最终 PASS / REVIEW 的单次评估路径；
  `final_decision` 只保留 workflow wrapper 职责。
- half 专属 content-mismatch selection override 已删除；content mismatch 只能成为候选 signal、
  rank input 或 decision gate input。
- `policies.formats.modules.import_format_module()` 已删除；format policy registry 改成显式表。
- `FormatSpec` 增加 physical traits，policy assembly 从 traits / mode posture 推导能力，不再用
  format 名称承载算法开关。
- `DetectionPolicy.source_parameters` 已移除；policy detail 输出精简 read model，不再整桶 dump
  runtime parameter aggregate。
- physical、evidence、guidance 和 foundation helper 只接收 subpolicy / 参数对象；不接完整
  `DetectionPolicy`，也不静默创建默认参数。
- robust grid 不再是 active separator refinement family；可靠 gap 分数归 separator support
  参数，model / grid evidence 只保留为 explanation。
- source naming contract 增加当前 schema / policy boundary / current-schema-only 约束，测试只保护当前
  V4.9 结构。
...
- policy registry lookup 收敛到 runtime / policies 边界；detection、report、debug 和
  analysis reuse 接收显式 runtime policy / subpolicy，不再从 `Detection` 反查 registry。
...
- runtime policy context 由 workflow 创建；检测、decision、finalization、report 和 debug
  沿运行流程接收 selected policy / subpolicy。
...
- final decision evidence、confidence cap 和 PASS / REVIEW 收敛到
...
- decision 包拆成 final reason helper、evidence summary、decision signals、decision gate 和 PASS / REVIEW
  applier。
...
- candidate assessment 的 gate 输出收敛为显式 `CandidateGateAssessment`；`blockers`
  和 `diagnostics` 从 `GateCheck` 派生，不再维护独立 blocker 词表或 generic fallback。
- detection 进一步改为物理事实驱动：新增 `frame_topology_evidence`、
  `separator_cross_axis_continuity`、`holder_texture_evidence` 和 per-side mixed-boundary
  outer evidence。frame 拓扑失败和 separator 横跨短轴连续性弱会进入 candidate gate；
  holder texture 只作为 guidance，不拥有 auto-pass 权限。
- candidate gate 拆清 `photo_size_consistency` 和 `frame_topology`；照片本体尺寸稳定继续由
  `photo_width_cv` 表达，separator width variation 仍只是 observed detail，不作为 blocker。
- base outer 的 side-boundary proposal 改为 `mixed_boundary`，每条边独立表达
  `black_border_to_white_holder`、`content_to_white_holder` 或 `mixed_or_uncertain`，
  不再假设真实图像四周都存在黑边。
- base scoring 的候选级 assessment 输出改为显式 `BaseDetectionAssessment`
  结果对象，字段化 `confidence`、`candidate_signals` 和 `detail`，避免匿名三元组解包
...
- separator gate profile / supplemental support helper 的输出改为显式
  `SeparatorGateSupportAssessment`，字段化 `ok` 和 `reason`，内部 gate 支持检查不再
  使用匿名 ok/reason tuple。
- gate check 输出统一为 `GateCheck`，字段化 `code`、`stage`、`bucket`、`passed`、
  `severity`、`signal` 和 `detail`，候选 gate 与最终 decision gate 共用同一表达。
- decision gate 输出改为显式 `DecisionGateAssessment`，字段化 `checks`、
  `final_review_reasons`、`reason_inputs` 和 `confidence_caps`；最终用户可见
  `final_review_reasons` 只由 decision gate 生成。
- `candidate_gate_failed` 是 gate 结果和 confidence-cap reason，不写回
  `candidate_signals`，也不作为候选物理证据 blocker 存储。
- candidate 子层的候选级物理 signal 读写收敛到 `detection.candidate.signals`；
  candidate assessment 不再把候选 signal 写入 final-looking `Detection.final_review_reasons`。
- candidate signal 存储从 `Detection.final_review_reasons` 拆出到
  `Detection.detail["candidate_signals"]`；`Detection.final_review_reasons` 只保留给
  decision 之后的最终用户可见 reason。
- candidate signal reader 不再 fallback 到 `Detection.final_review_reasons`，避免候选层重新读取
  final reason 字段。
- dual-lane mode 的模式失败 / lane 合并原因也改为通过 `detection.candidate.signals`
  读写；`detection.modes` 不再直接 mutate `Detection.final_review_reasons`。
...
- decision signals 新增 `candidate_source_detail`，明确区分
  `candidate_assessment.source` 和顶层 `candidate_source`；content-only、safety 和
  review-only 来源不再共享模糊 source。
- decision evidence summary 中 content quality 的 role 字段改为
  ...
  混淆。
- decision input 中删除旧 candidate-signal reducer；decision 只读取 candidate gate、
  evidence summary、decision signals 和 selection uncertainty inputs。
...
- 删除未使用的 `add_final_review_reason` helper；最终 reason 只保留 decision contract 的
  set-style 写入口。
- 旧 `overlap_risk` / `lucky_pass_risk` 语义已删除；output overlap 作为
  `output_overlap_evidence` 进入 decision signals，弱证据组合不再保留 lucky-pass gate。
- output-overlap evidence 前移到 decision 阶段生成；finalization 只消费已有 evidence detail
  来调整 output bleed，不再在 PASS / REVIEW 之后补充裁决输入。
- candidate selection 不再提前写 `candidate_competition_uncertain` 或执行 competition cap；
  它只记录 `selection_uncertainty_inputs`，最终 `candidate_competition_close` reason / cap 由
...
- content candidate assessment helper 和局部变量改用 diagnostics 命名；
  ...
  这类 final-looking 名称。
- base scoring 的局部输出命名改为 `candidate_signals`，明确它只写候选级 reason
  read model，不是最终 `final_review_reasons`。
- candidate table / selected candidate detail 的候选级信号字段改为 `candidate_signals`、
  `candidate_blockers`、`candidate_diagnostics`；decision 写入 `final_review_reasons` 和
  `final_confidence`。
- candidate plan / execution budget detail 的可靠性条件改为 `candidate_signals` 和
  `candidate_signals_ok`，runtime policy 字段改为 `requires_no_candidate_signals`。
...
- safety candidate 的 candidate gate signal、candidate cap 和 gate 改写从
...
- safety candidate detail 删除旧 `auto_pass_eligible` 字段，改为
  `candidate_gate_eligible`，避免候选级 gate 被误读成最终 PASS 资格。
- safety candidate detail 不再使用 `review_only` / `changes_pass_review` 这类 final-looking
  字段；候选 gate signal 写为 `safety_candidate_gate_blocked`。
- hard safety candidate 删除误导性的 `changes_final_decision` detail，改用
  `candidate_contract` 和 `candidate_gate_eligible` 表达候选契约。
...
- review-only / dual-lane mode detail 的模式级诊断改为 `mode_diagnostics` 和
  `candidate_signals`，不再在 mode detail 中输出 final-looking `final_review_reasons` 字段。
- review-only mode 不再在构造 `Detection` 时写入最终 `final_review_reasons`；模式失败原因保留为
  candidate / mode diagnostics，最终 REVIEW 原因仍由 decision contract 生成。
- close competition 的候选竞争阈值从 runtime candidate selection policy 汇入 decision
...
- decision summary 不再输出旧 reason 增量字段；最终全量原因只写入
  `final_review_reasons`。
- decision 子层的 final reason 归一化和写入收敛到 `detection.decision.reasons`；
  low-confidence context reason 不再直接 append `Detection.final_review_reasons`。
...
- report schema 中 output-overlap / deskew 可见细节从旧的 `finalization` 语义收敛到
  `diagnostics` section；finalization 继续只表示输出相邻几何和 bleed 调整。
- report schema 默认 sections 正式包含 `evidence`、`candidate_gate` 和 `decision_gate`，
  让已经构造好的审核 read-model 出现在报告中。
- policy / report 可见 gate stage 名称收敛为 `candidate_gate` 和 `decision_gate`；
  移除旧的 finalization / auto-pass gate 命名。
- runtime `GatePolicy` 移除未被消费的 `hard_final_review_reasons_block_auto` 字段；候选阻断
...
- review copy warning 从 “low confidence” 改为中性的 “review required”，避免把所有
  REVIEW 误写成低置信度。
- partial safe extra frames detail 只保留 `partial_edge_safety` canonical key；
...
- decision contract 移除未被实际裁决消费的 `candidate_policy` 报告字段；content-only /
  safety / review-only 规则由 decision signals 和 PASS / REVIEW applier 表达。
...
- decision policy 移除未被实际裁决消费的 content-only / safety review-only 布尔字段；
  candidate source -> decision signals -> decision applier 是唯一表达路径。
...
- candidate selection uncertainty input 删除 `recommended_final_review_reason` 字段；selection
  只记录候选竞争 signal，最终 reason 仍由 decision contract 生成。
...
- `135-dual/full` 使用独立 dual-lane mode detector；`135-dual/partial` 走通用
  review-only 保守路径。
- Debug Analysis 默认保持三联图；更细 evidence / gate / decision signal 解释写入 report detail。
...
- Runtime policy lookup is now owned by workflow / policies. Detection, decision,
  finalization, report, and debug receive explicit selected policy / subpolicy
  objects instead of looking up policy from a `Detection`.
...
- Final decision evidence, confidence caps, and PASS / REVIEW belong
...
- The decision package is split into final reason helpers, evidence summary,
  decision signals, decision gate, and the PASS / REVIEW applier.
...
- Policy assembly no longer uses `known_physical_risks` strings as parameter
  ...
  aspect.
- Candidate assessment gate output is now an explicit
  `CandidateGateAssessment`; `blockers` and `diagnostics` are derived from
  `GateCheck` results, with no independent blocker vocabulary or
  old candidate-gate input alias.
- Detection is more explicitly physical-fact driven: it now exposes
  `frame_topology_evidence`, `separator_cross_axis_continuity`,
  `holder_texture_evidence`, and per-side mixed-boundary outer evidence. Frame
  topology failures and weak cross-axis separator continuity enter candidate
  gate; holder texture remains guidance and has no auto-pass authority.
- Candidate gate separates `photo_size_consistency` from `frame_topology`.
  Photo size stability remains `photo_width_cv`; separator width variation is
  observed detail and not a blocker.
- The base-outer side-boundary proposal is now `mixed_boundary`; each side
  independently reports `black_border_to_white_holder`,
  `content_to_white_holder`, or `mixed_or_uncertain`, so outer proposal no
  longer assumes a black rim on every side.
- Base scoring candidate assessment now returns an explicit
  `BaseDetectionAssessment` result with `confidence`, `candidate_signals`,
...
- Separator gate profile and supplemental support helpers now return an
  explicit `SeparatorGateSupportAssessment` with `ok` and `reason` fields, so
  internal gate-support checks no longer use anonymous ok/reason tuples.
- Gate checks now use a shared `GateCheck` shape with `code`, `stage`, `bucket`,
  `passed`, `severity`, `signal`, and `detail` fields, so candidate gate and
  final decision gate share one structure.
- Decision gate output is now an explicit `DecisionGateAssessment` with
  `checks`, `final_review_reasons`, `reason_inputs`, and `confidence_caps`;
  user-visible final `final_review_reasons` are generated only by the decision gate.
- Candidate sublayers now route candidate-level physical signal reads and writes
  through `detection.candidate.signals`; candidate assessment no longer stores
  candidate signals in final-looking `Detection.final_review_reasons`.
- Candidate signal storage moved from `Detection.final_review_reasons` to
  `Detection.detail["candidate_signals"]`; `Detection.final_review_reasons` is now
  reserved for final user-visible reasons after the decision step.
- Candidate signal readers no longer fall back to `Detection.final_review_reasons`, so
  candidate layers cannot re-read final reason fields as candidate signals.
- Dual-lane mode failure and lane-merge reasons also route through
  `detection.candidate.signals`; `detection.modes` no longer directly mutates
...
- Decision signals now expose `candidate_source_detail`, separating
  `candidate_assessment.source` from top-level `candidate_source`, so
  content-only, safety, and review-only sources do not share a vague source.
- Decision evidence summary now reports content-quality score role as
  ...
  `content_score_role`.
- The old candidate-signal reducer has been removed from decision inputs;
  decision now reads candidate gate, evidence summary, decision signals, and
  selection uncertainty inputs.
- Output-overlap evidence is now attached during the decision step. Finalization
  only consumes existing evidence detail for output bleed and no longer adds
  PASS / REVIEW inputs after the decision.
- Candidate selection no longer writes `candidate_competition_uncertain` or
  applies the competition cap early. It only records `selection_uncertainty_inputs`;
  final `candidate_competition_close` reason / cap are generated by the decision
  contract.
- Candidate table / selected-candidate detail now uses `candidate_signals`,
...
- Base-scoring local output naming now uses `candidate_signals`, making it
  explicit that base scoring writes candidate-level physical signals, not
  final `final_review_reasons`.
- Candidate plan / execution-budget reliability detail now uses
  `candidate_signals` and `candidate_signals_ok`; runtime policy uses
...
- Safety-candidate gate signal, candidate cap, and gate rewrite moved from
  `candidate.plan` to `candidate.assessment`.
...
- Safety-candidate detail no longer emits the old `auto_pass_eligible` field;
  it uses `candidate_gate_eligible` so candidate-level gate detail is not
  mistaken for final PASS eligibility.
- Safety-candidate detail no longer uses final-looking `review_only` /
  `changes_pass_review` fields; the candidate gate signal is reported as
  `safety_candidate_gate_blocked`.
- Hard-safety candidate detail no longer emits the misleading
  `changes_final_decision`; it uses `candidate_contract` and
  `candidate_gate_eligible` to describe the candidate contract.
...
- Review-only / dual-lane mode detail now uses `mode_diagnostics` and
  `candidate_signals` for mode-level diagnostics, instead of final-looking
  `final_review_reasons` fields inside mode detail.
- Review-only mode no longer writes final `final_review_reasons` while constructing
...
- The close-competition candidate threshold now flows from runtime candidate
...
- Decision summary no longer emits old incremental reason fields. The final full
  list is reported only as `final_review_reasons`.
- Decision sublayers now route final reason normalization and writes through
  ...
  `Detection.final_review_reasons` directly.
...
- Report, debug, and export are now explicit output read-models. They consume
  ...
  as `unknown` / `UNKNOWN`.
- Report-visible output-overlap / deskew details now live under the `diagnostics` section
  instead of the old finalization-shaped read-model name.
- Report schema default sections now include `evidence`, `candidate_gate`, and
  `decision_gate`, making the already-constructed audit read-model visible in reports.
- Policy/report-visible gate stage names now use `candidate_gate` and
  `decision_gate`, retiring finalization / auto-pass shaped gate names.
- Runtime `GatePolicy` no longer carries the unused
  `hard_final_review_reasons_block_auto` field; candidate blockers live in candidate
...
- Partial safe extra frames detail now keeps only the canonical
  `partial_edge_safety` key; the old `partial_extra_holder_frames` alias
  has been removed.
...
- The decision contract no longer reports the unused `candidate_policy` field;
  content-only, safety, and review-only behavior is expressed by decision signals and
...
- The decision contract no longer carries output / diagnostics policy report
  ...
  runtime policies.
- Decision policy no longer carries unused content-only / safety review-only
  boolean fields. Candidate source -> decision signals -> decision
...
- Candidate selection uncertainty input no longer carries
  `recommended_final_review_reason`; selection records only candidate-competition
  signals, and final reasons remain generated by the decision contract.
...
- Content scoring uses content-protection semantics: safe overcut and empty
  frames are not negative evidence; harm to real content is a content-integrity signal.
...
- Debug Analysis keeps a three-panel default; richer evidence / gate / decision signal
