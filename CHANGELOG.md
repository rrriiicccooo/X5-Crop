...
- `python3 -m unittest discover -s tools/tests` 通过 54 个 unit / contract tests。
- `git diff --check` 通过。
...
- Decision summary no longer emits the old `review_reasons_added` field; final
  reason additions are reported as `final_review_reasons_added`.
...
- `python3 -m unittest discover -s tools/tests` passed 54 unit / contract tests.
