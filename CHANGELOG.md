...
- runtime decision policy 已从 finalization policy 中拆出；confidence cap、post-check review
...
- decision summary 不再输出旧 `review_reasons_added` 字段；最终 reason 增量写入
  `final_review_reasons_added`，包括最后补充的 decision post-check reason。
...
- Runtime decision policy has been split out of finalization policy. Confidence
  caps, post-check review reasons, and the outer-content alignment evidence switch now
