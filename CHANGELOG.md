...
- output overlap 从 final review blocker 收敛为 output-protection evidence：可由 bleed
  保护的 overlap 写入 `output_overlap_protected_by_bleed` 并提高输出长轴 bleed；只有
...
- Candidate lifecycle is stricter: `candidate.plan` only declares count, offset,
  source descriptors, and execution budget; execution-budget detail and batch
  assessment now live in dedicated helpers, while build, assessment, and
  selection remain separate sublayers.
...
- Output overlap is now output-protection evidence instead of a final review
  blocker: bleed-coverable overlap is written as
  `output_overlap_protected_by_bleed` and increases long-axis output bleed; only
