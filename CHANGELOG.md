...
当前 active 脚本：`X5_Crop.py` V3.6.8
...
| 版本 | 状态 | 摘要 |
|---|---|---|
| V3.6.8 | 当前 active 开发版 | 将 single-anchor 精确组合闸门改成 `lucky_pass_risk_score`，避免规则看起来像为单张图定制。 |
...
Current active script: `X5_Crop.py` V3.6.8
...
|---|---|---|
| V3.6.8 | Current active development | Replaces the exact single-anchor gate with `lucky_pass_risk_score`, so the rule no longer reads as tailored to one image. |
