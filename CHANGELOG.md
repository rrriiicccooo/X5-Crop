...
- decision gate 输出改为显式 `DecisionGateAssessment`，字段化 `checks`、
  ...
  `review_reasons` 只由 decision gate 生成。
...
- candidate 子层的候选级物理 signal 读写收敛到 `detection.candidate.signals`；
  candidate assessment 不再把候选 signal 写入 final-looking `Detection.review_reasons`。
- candidate signal 存储从 `Detection.review_reasons` 拆出到
  `Detection.detail["candidate_signals"]`；`Detection.review_reasons` 只保留给
  decision 之后的最终用户可见 reason。
- candidate signal reader 不再 fallback 到 `Detection.review_reasons`，避免候选层重新读取
  final reason 字段。
...
- review-only / dual-lane mode detail 的模式级诊断改为 `mode_diagnostics` 和
  `candidate_signals`，不再在 mode detail 中输出 final-looking `review_reasons` 字段。
...
- Decision gate output is now an explicit `DecisionGateAssessment` with
  ...
  user-visible final `review_reasons` are generated only by the decision gate.
- Candidate sublayers now route candidate-level physical signal reads and writes
  ...
  candidate signals in final-looking `Detection.review_reasons`.
- Candidate signal storage moved from `Detection.review_reasons` to
  `Detection.detail["candidate_signals"]`; `Detection.review_reasons` is now
  reserved for final user-visible reasons after the decision step.
- Candidate signal readers no longer fall back to `Detection.review_reasons`, so
