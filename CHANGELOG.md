...
- output bleed、output overlap 和 approved geometry adjustment 已与最终决策输入分开；finalization
  对 output detection 副本做几何调整，并同时记录 `decision_geometry` 与 `output_geometry`。
- decision evidence policy 已从 format-id override 表收敛为 physical trait 推导；format 名字只作为
  `FormatSpec` 查询入口。
- policy 参数 topology 仍以 `FormatParameters` 聚合对象作为装配入口，但旧 per-format
  module registry 和 string override mapping 已删除；后续参数继续向更细分组 dataclass 收敛。
...
- Report / debug / cache reuse read the current schema only; old decision
  contract helpers, policy-detail fallback, cached final-reason fallback, and
  missing-decision-summary final-reason fallback have been removed.
...
- Output bleed, output overlap, and approved geometry adjustment are separate
  from final decision inputs. Finalization adjusts an output detection clone and
  records `decision_geometry` separately from `output_geometry`.
...
- Policy parameters still use `FormatParameters` as the assembly aggregate, but
  the old per-format module registry and string override mapping have been
  removed. Future cleanup can continue splitting the aggregate into smaller
  grouped dataclasses.
