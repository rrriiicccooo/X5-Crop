...
- detection 架构表述升级为协同证据图：physical structure、content guidance、
  candidate build、evidence、assessment、selection / extension、decision 和 finalization
  分层协作，不再把 outer / separator / content 当作互相隔离的检测 oracle。
- outer correction eligibility 从 physical 层迁到 `candidate.extension`；physical correction
  不再读取 `candidate_assessment`，只生成和约束 corrected outer proposal。
...
- outer proposal detail 现在随 `OuterCandidate` 流入 `outer_candidate_detail` 和
  candidate table；candidate build 不再按 candidate 名字反向重算 side-boundary 证据。
...
- Detection is now documented as a cooperative evidence graph: physical
  structure, content guidance, candidate build, evidence, assessment,
  selection / extension, decision, and finalization collaborate without making
  outer / separator / content separate oracles.
- Outer correction eligibility now belongs to `candidate.extension`; physical
  correction no longer reads `candidate_assessment` and only proposes /
  constrains corrected outer geometry.
