...
- 当前状态仅为 `architecture closure candidate`。新的 Codex 任务必须完整重跑同一双轮
  计划，之后才能标记 `architecture closed`。
...
- selected candidate 的 content containment / outer alignment evidence 在 evidence 层完成；
  DecisionGate 只消费显式 evidence 和 decision contract，不再接收完整 runtime policy。
...
- partial auto 增加 count-independent separator preflight：hard separator 数量优先排列 count，
  实测 separator 位置生成连续 placement；可信 placement 后停止等价 offset，证据不足时才展开
  其余 hypotheses。default count 不再运行物理上等价的五个 offset。
- candidate execution budget 不再为 extension 重建 primary candidates；full-width 与 content-guided
  ...
  proposal、observed-width profile 和 per-outer content threshold 复用；width profile 的短轴统计
  使用已声明的 sampling budget。
...
Architecture closure candidate (2026-07-11):
...
- This is only an `architecture closure candidate`. A new Codex task must rerun
  the complete two-audit plan before marking `architecture closed`.
...
- The evidence layer completes content-containment and outer-alignment evidence
  for the selected candidate; DecisionGate consumes explicit evidence and the
  decision contract, never a full runtime policy.
...
- Partial auto now runs a count-independent separator preflight. Hard separator
  count orders hypotheses, measured separator positions derive continuous
  placement, and trusted placement stops equivalent offset work. Nominal-count
  hypotheses no longer execute five physically identical offsets.
- Candidate execution no longer rebuilds primary candidates for extension work.
  ...
  reuses base outer, complete outer proposal, observed-width profiles, and
  per-outer content thresholds; width measurement honors its declared short-axis
  sampling budget.
