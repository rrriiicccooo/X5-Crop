...
- frame-box width fallback 不再触发 `photo_width_unstable`、partial-holder 照片宽度阻断
  或 final photo-width gate；这些 gate 只消费 `photo_edges` 来源的照片宽度证据。
...
- Width scoring now separates `photo_width_cv`, `frame_box_width_cv`, and
  `separator_width_cv`: photo image-region size stability is scoring / gate
  evidence, while separator-width variation is evidence detail rather than a
  direct instability penalty.
