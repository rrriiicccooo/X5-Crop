...
- content support score 和 content quality score 拆清：前者表示真实内容完整包含的支持度，
  ...
  当 detail 明确给出 `content_harm_risk` 时，content support score 不再被旧
  `support: ok` fallback 覆盖。
...
- frame-box width fallback 不再触发 `photo_width_unstable`、support score、
...
- scoring / gate / partial-holder / decision contract 的宽度稳定性 policy 字段改为
  `photo_width_*` 命名；`width_cv` 仅保留为报告兼容字段或 separator / gap 几何测量。
...
- Content support score and content quality score are now separated: support
  ...
  no longer overridden by the legacy `support: ok` fallback.
...
- Width scoring now separates `photo_width_cv`, `frame_box_width_cv`, and
  `separator_width_cv`: only `photo_edges` photo-width evidence may feed support
  scores, gate support, risk credit, or final photo-width gates; frame-box width
  fallback and separator-width variation remain diagnostic detail.
- Scoring / gate / partial-holder / decision-contract width-stability policy
  fields now use `photo_width_*` names; plain `width_cv` remains report
  compatibility or separator / gap geometry measurement wording.
